# ElysiaClaw ACP Bridge

This document describes how the ElysiaClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`elysiaclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
ElysiaClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing ElysiaClaw Gateway
...
- ACP client spawns `elysiaclaw acp` and speaks ACP messages over stdio.
...
`elysiaclaw acp` resolves the Gateway URL and auth from CLI flags or config:
