...
BASE_IMAGE="${BASE_IMAGE:-elysiaclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-elysiaclaw-sandbox-common:bookworm-slim}"
...
ELYSIACLAW_DOCKER_BUILD_USE_BUILDX="${ELYSIACLAW_DOCKER_BUILD_USE_BUILDX:-0}"
ELYSIACLAW_DOCKER_BUILD_CACHE_FROM="${ELYSIACLAW_DOCKER_BUILD_CACHE_FROM:-}"
ELYSIACLAW_DOCKER_BUILD_CACHE_TO="${ELYSIACLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${ELYSIACLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${ELYSIACLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${ELYSIACLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${ELYSIACLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${ELYSIACLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=elysiaclaw.sandbox=1)
NOTE
