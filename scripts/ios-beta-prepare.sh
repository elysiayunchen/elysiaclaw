...
- regenerates apps/ios/ElysiaClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${ELYSIACLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid ELYSIACLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid ELYSIACLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid ELYSIACLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid ELYSIACLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing ELYSIACLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(ELYSIACLAW_URL_SLASH)$(ELYSIACLAW_URL_SLASH)#g'
)"
...
ELYSIACLAW_CODE_SIGN_STYLE = Automatic
ELYSIACLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
ELYSIACLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
ELYSIACLAW_APP_BUNDLE_ID = ai.elysiaclaw.client
ELYSIACLAW_SHARE_BUNDLE_ID = ai.elysiaclaw.client.share
ELYSIACLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.elysiaclaw.client.activitywidget
ELYSIACLAW_WATCH_APP_BUNDLE_ID = ai.elysiaclaw.client.watchkitapp
ELYSIACLAW_WATCH_EXTENSION_BUNDLE_ID = ai.elysiaclaw.client.watchkitapp.extension
ELYSIACLAW_APP_PROFILE =
ELYSIACLAW_SHARE_PROFILE =
ELYSIACLAW_PUSH_TRANSPORT = relay
ELYSIACLAW_PUSH_DISTRIBUTION = official
ELYSIACLAW_URL_SLASH = /
ELYSIACLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
ELYSIACLAW_PUSH_APNS_ENVIRONMENT = production
EOF
