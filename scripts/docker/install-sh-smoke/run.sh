...
INSTALL_URL="${ELYSIACLAW_INSTALL_URL:-https://elysiaclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ELYSIACLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ELYSIACLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="elysiaclaw"
PACKAGE_NAME="${ELYSIACLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ELYSIACLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ELYSIACLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ELYSIACLAW_INSTALL_LATEST_OUT:-}"
fi
