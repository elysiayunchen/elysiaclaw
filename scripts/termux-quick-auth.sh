...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/elysiaclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ElysiaClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/elysiaclaw/scripts/mobile-reauth.sh'" --id elysiaclaw-auth
        ;;
    *)
