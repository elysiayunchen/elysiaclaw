...
const EXPECTED_REPOSITORY_URL = "https://github.com/elysiaclaw/elysiaclaw";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "elysiaclaw") {
    errors.push(`package.json name must be "elysiaclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.elysiaclaw !== "elysiaclaw.mjs") {
    errors.push(
      `package.json bin.elysiaclaw must be "elysiaclaw.mjs"; found "${pkg.bin?.elysiaclaw ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`elysiaclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `elysiaclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
