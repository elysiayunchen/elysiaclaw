...
#
# Suggested cron: */30 * * * * /home/admin/elysiaclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send ElysiaClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., elysiaclaw-alerts)
...
STATE_FILE="$HOME/.elysiaclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ElysiaClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use elysiaclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ElysiaClaw to $NOTIFY_PHONE..."
            elysiaclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ElysiaClaw is down. Run: ssh l36 '~/elysiaclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
