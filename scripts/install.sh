...
# ElysiaClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://elysiaclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one ElysiaClaw."
...
GUM_VERSION="${ELYSIACLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 ElysiaClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://elysiaclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$ELYSIACLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.elysiaclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_elysiaclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.elysiaclaw-* "$npm_root"/elysiaclaw 2>/dev/null || true
}

extract_elysiaclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_elysiaclaw_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/elysiaclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/elysiaclaw"|"/usr/local/bin/elysiaclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/elysiaclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale elysiaclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing elysiaclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing ElysiaClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_elysiaclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "ElysiaClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*elysiaclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_elysiaclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_elysiaclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "ElysiaClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an elysiaclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "ELYSIACLAW_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "ELYSIACLAW_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "ELYSIACLAW_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "ELYSIACLAW_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "ELYSIACLAW_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "ELYSIACLAW_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "ELYSIACLAW_BETA" "CLAWDBOT_BETA"
map_legacy_env "ELYSIACLAW_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "ELYSIACLAW_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "ELYSIACLAW_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "ELYSIACLAW_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "ELYSIACLAW_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "ELYSIACLAW_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${ELYSIACLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${ELYSIACLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((ELYSIACLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${ELYSIACLAW_NO_ONBOARD:-0}
NO_PROMPT=${ELYSIACLAW_NO_PROMPT:-0}
DRY_RUN=${ELYSIACLAW_DRY_RUN:-0}
INSTALL_METHOD=${ELYSIACLAW_INSTALL_METHOD:-}
ELYSIACLAW_VERSION=${ELYSIACLAW_VERSION:-latest}
USE_BETA=${ELYSIACLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/elysiaclaw"
GIT_DIR=${ELYSIACLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${ELYSIACLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${ELYSIACLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${ELYSIACLAW_VERBOSE:-0}"
VERIFY_INSTALL="${ELYSIACLAW_VERIFY_INSTALL:-0}"
ELYSIACLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
ElysiaClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://elysiaclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/elysiaclaw)
...
Environment variables:
  ...
  ELYSIACLAW_VERSION=latest|next|<semver>
  ELYSIACLAW_BETA=0|1
  ELYSIACLAW_GIT_DIR=...
  ELYSIACLAW_GIT_UPDATE=0|1
  ELYSIACLAW_NO_PROMPT=1
  ELYSIACLAW_VERIFY_INSTALL=1
  ELYSIACLAW_DRY_RUN=1
  ELYSIACLAW_NO_ONBOARD=1
  ELYSIACLAW_VERBOSE=1
  ELYSIACLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://elysiaclaw.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://elysiaclaw.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://elysiaclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://elysiaclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected ElysiaClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a ElysiaClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_elysiaclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"elysiaclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://elysiaclaw.ai/install.sh | bash"
}
...
ensure_elysiaclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/elysiaclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/elysiaclaw" ]]; then
        ln -sf "$npm_root/elysiaclaw/dist/entry.js" "${npm_bin}/elysiaclaw"
        ui_info "Created elysiaclaw bin link at ${npm_bin}/elysiaclaw"
    fi
    return 0
}

# Check for existing ElysiaClaw installation
check_existing_elysiaclaw() {
    if [[ -n "$(type -P elysiaclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing ElysiaClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make elysiaclaw show as \"command not found\" in new terminals."
...
warn_elysiaclaw_not_found() {
    ui_warn "Installed, but elysiaclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t elysiaclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named elysiaclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_elysiaclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P elysiaclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/elysiaclaw" ]]; then
        echo "${npm_bin}/elysiaclaw"
        return 0
    fi
...
install_elysiaclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/elysiaclaw/elysiaclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing ElysiaClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing ElysiaClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning ElysiaClaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building ElysiaClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/elysiaclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/elysiaclaw"
    ui_success "ElysiaClaw wrapper installed to \$HOME/.local/bin/elysiaclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view elysiaclaw dist-tags.beta 2>/dev/null || true)"
...
install_elysiaclaw() {
    local package_name="elysiaclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            ELYSIACLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="elysiaclaw"
        else
            ELYSIACLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${ELYSIACLAW_VERSION}" ]]; then
        ELYSIACLAW_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${ELYSIACLAW_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing ElysiaClaw v${resolved_version}"
    else
        ui_info "Installing ElysiaClaw (${ELYSIACLAW_VERSION})"
    fi
    local install_spec=""
    if [[ "${ELYSIACLAW_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${ELYSIACLAW_VERSION}"
    fi

    if ! install_elysiaclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_elysiaclaw_paths
        install_elysiaclaw_npm "${install_spec}"
    fi

    if [[ "${ELYSIACLAW_VERSION}" == "latest" && "${package_name}" == "elysiaclaw" ]]; then
        if ! resolve_elysiaclaw_bin &> /dev/null; then
            ui_warn "npm install elysiaclaw@latest failed; retrying elysiaclaw@next"
            cleanup_npm_elysiaclaw_paths
            install_elysiaclaw_npm "elysiaclaw@next"
        fi
    fi

    ensure_elysiaclaw_bin_link || true

    ui_success "ElysiaClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${ELYSIACLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_elysiaclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (elysiaclaw not on PATH yet)"
        warn_elysiaclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${ELYSIACLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.elysiaclaw/workspace-${profile}"
    else
        echo "${HOME}/.elysiaclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${ELYSIACLAW_CONFIG_PATH:-$HOME/.elysiaclaw/elysiaclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run elysiaclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but elysiaclaw not on PATH; skipping onboarding"
        warn_elysiaclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run elysiaclaw onboard to retry"
        return
    }
}
...
if ! declare -F extract_elysiaclaw_semver >/dev/null 2>&1; then
...
resolve_elysiaclaw_version() {
    ...
    local claw="${ELYSIACLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v elysiaclaw &> /dev/null; then
        claw="$(command -v elysiaclaw)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_elysiaclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/elysiaclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/elysiaclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: elysiaclaw not on PATH yet"
        warn_elysiaclaw_not_found
        return 1
    fi

    run_quiet_step "Checking ElysiaClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: elysiaclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_elysiaclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found ElysiaClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set ELYSIACLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_elysiaclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing ElysiaClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g elysiaclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g elysiaclaw 2>/dev/null || true
        ...
        install_elysiaclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/elysiaclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/elysiaclaw"
    ...
    ELYSIACLAW_BIN="$(resolve_elysiaclaw_bin || true)"
    ...
    installed_version=$(resolve_elysiaclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 ElysiaClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 ElysiaClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/elysiaclaw"
        ui_kv "Update command" "elysiaclaw update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://elysiaclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running elysiaclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                ELYSIACLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                ELYSIACLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                ELYSIACLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run elysiaclaw doctor and elysiaclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run elysiaclaw onboard later"
        else
            local config_path="${ELYSIACLAW_CONFIG_PATH:-$HOME/.elysiaclaw/elysiaclaw.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (elysiaclaw not on PATH yet)"
                    warn_elysiaclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run elysiaclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v elysiaclaw &> /dev/null; then
        local claw="${ELYSIACLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_elysiaclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (elysiaclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if ELYSIACLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: elysiaclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${ELYSIACLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
