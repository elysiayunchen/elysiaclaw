#!/bin/bash
# Setup ElysiaClaw Auth Management System
...
echo "=== ElysiaClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'elysiaclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ElysiaClaw message: Send warning via ElysiaClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/elysiaclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/elysiaclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/elysiaclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now elysiaclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status elysiaclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
