...
# Build and bundle ElysiaClaw into a minimal .app we can open.
# Outputs to dist/ElysiaClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/ElysiaClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="ElysiaClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.elysiaclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/elysiaclaw/elysiaclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ElysiaClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ElysiaClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ElysiaClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ElysiaClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ElysiaClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ElysiaClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ElysiaClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/ElysiaClaw/Resources/ElysiaClaw.icns" "$APP_ROOT/Contents/Resources/ElysiaClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/ElysiaClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ElysiaClawKit resources"
ELYSIACLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ElysiaClawKit_ElysiaClawKit.bundle"
if [ -d "$ELYSIACLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ElysiaClawKit_ElysiaClawKit.bundle"
  cp -R "$ELYSIACLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ElysiaClawKit_ElysiaClawKit.bundle"
else
  echo "WARN: ElysiaClawKit resource bundle not found at $ELYSIACLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running ElysiaClaw"
killall -q ElysiaClaw 2>/dev/null || true
