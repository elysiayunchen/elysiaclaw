...
function stripWorkspaceelysiaclawDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.elysiaclaw !== "workspace:*") {
    return false;
  }
  delete devDeps.elysiaclaw;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceelysiaclawDevDependency(pkg);
