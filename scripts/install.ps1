# elysiaclaw Installer for Windows (PowerShell)
# Usage: iwr -useb https://elysiaclaw.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://elysiaclaw.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\elysiaclaw",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 elysiaclaw Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one elysiaclaw.$NC" -Level info
    Write-Host ""
}
...
function Install-elysiaclawNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing elysiaclaw (elysiaclaw@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g elysiaclaw@$Version --no-fund --no-audit 2>&1
        Write-Host "elysiaclaw installed" -Level success
        return $true
    } catch {
...
function Install-elysiaclawGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing elysiaclaw from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/elysiaclaw/elysiaclaw.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\elysiaclaw\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\elysiaclaw.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install elysiaclaw from git to $GitDir" -Level info
        } else {
            Install-elysiaclawGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install elysiaclaw via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-elysiaclawNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'elysiaclaw onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 elysiaclaw installed successfully!" -Level success
}
