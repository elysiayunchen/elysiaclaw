...
IMAGE_NAME="elysiaclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="elysiaclaw-net-e2e-$$"
GW_NAME="elysiaclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "ELYSIACLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "ELYSIACLAW_SKIP_CHANNELS=1" \
  -e "ELYSIACLAW_SKIP_GMAIL_WATCHER=1" \
  -e "ELYSIACLAW_SKIP_CRON=1" \
  -e "ELYSIACLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
