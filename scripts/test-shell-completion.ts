 ...
 * `elysiaclaw update`. Use it to verify the completion installation flow
...
\`elysiaclaw update\`, \`elysiaclaw doctor\`, and \`elysiaclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# ElysiaClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`elysiaclaw completion --install\` later to enable.`));
    return;
  }
