#!/usr/bin/env bash
# ClawDock - Docker helpers for ElysiaClaw
# Inspired by Simon Willison's "Running ElysiaClaw in Docker"
# https://til.simonwillison.net/llms/elysiaclaw-docker
#
# Installation:
...
# Common paths to check for ElysiaClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/elysiaclaw"
  "${HOME}/workspace/elysiaclaw"
  "${HOME}/projects/elysiaclaw"
  "${HOME}/dev/elysiaclaw"
  "${HOME}/code/elysiaclaw"
  "${HOME}/src/elysiaclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found ElysiaClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/elysiaclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ ElysiaClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/elysiaclaw/elysiaclaw.git ~/elysiaclaw"
    echo "  cd ~/elysiaclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ELYSIACLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d elysiaclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart elysiaclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f elysiaclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.elysiaclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec elysiaclaw-gateway \
    bash -c 'echo "alias elysiaclaw=\"./elysiaclaw.mjs\"" > /tmp/.bashrc_elysiaclaw && bash --rcfile /tmp/.bashrc_elysiaclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm elysiaclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build elysiaclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ELYSIACLAW_GATEWAY_TOKEN=$token" elysiaclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" elysiaclaw-gateway \
    bash -c './elysiaclaw.mjs config set gateway.remote.token "$TOKEN" && ./elysiaclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec elysiaclaw-gateway \
    bash -c "./elysiaclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart elysiaclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm elysiaclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec elysiaclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'elysiaclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for ElysiaClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (elysiaclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to elysiaclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.elysiaclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'elysiaclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'elysiaclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.elysiaclaw.ai${_CLR_RESET}"
  echo ""
}
