---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ElysiaClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ElysiaClaw cron scheduling for periodic checks, or version status checks on a machine running ElysiaClaw (laptop, workstation, Pi, VPS).
---

# ElysiaClaw Host Hardening

## Overview

Assess and harden the host running ElysiaClaw, then align it to a user-defined risk tolerance without breaking access. Use ElysiaClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ElysiaClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. ElysiaClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ElysiaClaw can access sensitive data.
...
### 2) Run ElysiaClaw security audits (read-only)

As part of the default read-only checks, run `elysiaclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `elysiaclaw security audit` (faster, non-probing)
2. `elysiaclaw security audit --json` (structured output)

Offer to apply ElysiaClaw safe defaults (numbered):

1. `elysiaclaw security audit --fix`

Be explicit that `--fix` only tightens ElysiaClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ElysiaClaw version/update status (read-only)

As part of the default read-only checks, run `elysiaclaw update status`.
...
- Credential hygiene notes (location of ElysiaClaw creds, prefer disk encryption)
...
- ElysiaClaw security audit (re-run)
...
After ElysiaClaw install or first hardening pass, run at least one baseline audit and version check:

- `elysiaclaw security audit`
- `elysiaclaw security audit --deep`
- `elysiaclaw update status`

Ongoing monitoring is recommended. Use the ElysiaClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `elysiaclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `elysiaclaw update status`
...
Before creating, `elysiaclaw cron list` and match on exact `name`. If found, `elysiaclaw cron edit <id> ...`.
If not found, `elysiaclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `elysiaclaw update status` (preferred for source checkouts and channels)
2. `npm view elysiaclaw version` (published npm version)

## ElysiaClaw command accuracy

Use only supported commands and flags:

- `elysiaclaw security audit [--deep] [--fix] [--json]`
- `elysiaclaw status` / `elysiaclaw status --deep`
- `elysiaclaw health --json`
- `elysiaclaw update status`
- `elysiaclaw cron add|list|runs|run`

Do not invent CLI flags or imply ElysiaClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ElysiaClaw compaction:
