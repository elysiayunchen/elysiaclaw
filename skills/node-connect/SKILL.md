---
name: node-connect
description: Diagnose ElysiaClaw node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify ElysiaClaw is advertising that route, then fix pairing/auth.
...
- whether `elysiaclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `elysiaclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
elysiaclaw config get gateway.mode
elysiaclaw config get gateway.bind
elysiaclaw config get gateway.tailscale.mode
elysiaclaw config get gateway.remote.url
elysiaclaw config get gateway.auth.mode
elysiaclaw config get gateway.auth.allowTailscale
elysiaclaw config get plugins.entries.device-pair.config.publicUrl
elysiaclaw qr --json
elysiaclaw devices list
elysiaclaw nodes status
```

If this ElysiaClaw instance is pointed at a remote gateway, also run:

```bash
elysiaclaw qr --remote --json
```
...
`elysiaclaw qr --json` success means:
...
If `elysiaclaw qr --json` says `Gateway is only bound to loopback`:
...
- `elysiaclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run elysiaclaw qr again, rescan, then approve the pending device pairing.`
