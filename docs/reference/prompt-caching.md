...
- With Anthropic API-key auth profiles, ElysiaClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, ElysiaClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
ElysiaClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.elysiaclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$ELYSIACLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `ELYSIACLAW_CACHE_TRACE=1` enables cache tracing.
- `ELYSIACLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `ELYSIACLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `ELYSIACLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `ELYSIACLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
