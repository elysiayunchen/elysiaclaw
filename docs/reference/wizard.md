...
This is the full reference for the `elysiaclaw onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.elysiaclaw/credentials/oauth.json`; auth profiles live in `~/.elysiaclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.elysiaclaw/credentials/oauth.json` (or `$ELYSIACLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.elysiaclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `elysiaclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `elysiaclaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `elysiaclaw health`.
    - Tip: `elysiaclaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export ELYSIACLAW_GATEWAY_TOKEN="your-token"
elysiaclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env ELYSIACLAW_GATEWAY_TOKEN
```
...
- Stores it under `~/.elysiaclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.elysiaclaw/elysiaclaw.json`:
...
`elysiaclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.elysiaclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.elysiaclaw/agents/<agentId>/sessions/`.
