...
ElysiaClaw features that can generate provider usage or paid API calls.
...
- `elysiaclaw status --usage` and `elysiaclaw channels list` show provider **usage windows**
...
ElysiaClaw can pick up credentials from:
...
- `elysiaclaw status --usage`
- `elysiaclaw models status --json`
...
`elysiaclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
