...
通过一键模板在 Railway 上部署 elysiaclaw，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/elysiaclaw` — 控制面板 UI
...
- `elysiaclaw_STATE_DIR=/data/.elysiaclaw`（推荐）
- `elysiaclaw_WORKSPACE_DIR=/data/workspace`（推荐）
- `elysiaclaw_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 elysiaclaw 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
