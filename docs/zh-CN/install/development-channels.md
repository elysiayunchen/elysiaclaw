...
当你使用 `--channel` **显式**切换渠道时，ElysiaClaw 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/elysiaclaw`，可通过 `ELYSIACLAW_GIT_DIR` 覆盖），
...
当你使用 `elysiaclaw update` 切换渠道时，ElysiaClaw 还会同步插件来源：
