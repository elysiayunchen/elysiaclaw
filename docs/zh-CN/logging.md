...
你可以在 `~/.elysiaclaw/elysiaclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.elysiaclaw/elysiaclaw.json` 的 `logging` 下。
...
- ElysiaClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `elysiaclaw plugins enable diagnostics-otel` 启用插件。
...
- `elysiaclaw.tokens`（计数器，属性：`elysiaclaw.token`、`elysiaclaw.channel`、`elysiaclaw.provider`、`elysiaclaw.model`）
- `elysiaclaw.cost.usd`（计数器，属性：`elysiaclaw.channel`、`elysiaclaw.provider`、`elysiaclaw.model`）
- `elysiaclaw.run.duration_ms`（直方图，属性：`elysiaclaw.channel`、`elysiaclaw.provider`、`elysiaclaw.model`）
- `elysiaclaw.context.tokens`（直方图，属性：`elysiaclaw.context`、`elysiaclaw.channel`、`elysiaclaw.provider`、`elysiaclaw.model`）

消息流：

- `elysiaclaw.webhook.received`（计数器，属性：`elysiaclaw.channel`、`elysiaclaw.webhook`）
- `elysiaclaw.webhook.error`（计数器，属性：`elysiaclaw.channel`、`elysiaclaw.webhook`）
- `elysiaclaw.webhook.duration_ms`（直方图，属性：`elysiaclaw.channel`、`elysiaclaw.webhook`）
- `elysiaclaw.message.queued`（计数器，属性：`elysiaclaw.channel`、`elysiaclaw.source`）
- `elysiaclaw.message.processed`（计数器，属性：`elysiaclaw.channel`、`elysiaclaw.outcome`）
- `elysiaclaw.message.duration_ms`（直方图，属性：`elysiaclaw.channel`、`elysiaclaw.outcome`）

队列 + 会话：

- `elysiaclaw.queue.lane.enqueue`（计数器，属性：`elysiaclaw.lane`）
- `elysiaclaw.queue.lane.dequeue`（计数器，属性：`elysiaclaw.lane`）
- `elysiaclaw.queue.depth`（直方图，属性：`elysiaclaw.lane` 或 `elysiaclaw.channel=heartbeat`）
- `elysiaclaw.queue.wait_ms`（直方图，属性：`elysiaclaw.lane`）
- `elysiaclaw.session.state`（计数器，属性：`elysiaclaw.state`、`elysiaclaw.reason`）
- `elysiaclaw.session.stuck`（计数器，属性：`elysiaclaw.state`）
- `elysiaclaw.session.stuck_age_ms`（直方图，属性：`elysiaclaw.state`）
- `elysiaclaw.run.attempt`（计数器，属性：`elysiaclaw.attempt`）
...
- `elysiaclaw.webhook.error`
  - `elysiaclaw.channel`、`elysiaclaw.webhook`、`elysiaclaw.chatId`、`elysiaclaw.error`
- `elysiaclaw.message.processed`
  - `elysiaclaw.channel`、`elysiaclaw.outcome`、`elysiaclaw.chatId`、`elysiaclaw.messageId`、`elysiaclaw.sessionKey`、`elysiaclaw.sessionId`、`elysiaclaw.reason`
- `elysiaclaw.session.stuck`
  - `elysiaclaw.state`、`elysiaclaw.ageMs`、`elysiaclaw.queueDepth`、`elysiaclaw.sessionKey`、`elysiaclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `elysiaclaw doctor`。
