...
2. 从旧机器复制 `$OPENCLAW_STATE_DIR`（默认：`~/.elysiaclaw`）。
...
所有内容位于 `$OPENCLAW_STATE_DIR`（默认：`~/.elysiaclaw`）下：

| 路径                                                            | 用途                                                 |
| --------------------------------------------------------------- | ---------------------------------------------------- |
| `$OPENCLAW_STATE_DIR/elysiaclaw.json`                             | 主配置（JSON5）                                      |
...
ElysiaClaw 从 `$OPENCLAW_CONFIG_PATH`（默认：`~/.elysiaclaw/elysiaclaw.json`）读取可选的 **JSON5** 配置：
...
- 当前工作目录下的 `.env`
- `~/.elysiaclaw/.env`（即 `$OPENCLAW_STATE_DIR/.env`）的全局回退 `.env`
...
- 如果你使用了配置文件（`--profile` / `OPENCLAW_PROFILE`），重置每个状态目录（默认为 `~/.elysiaclaw-<profile>`）。
...
- macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`（默认：`~/.elysiaclaw/logs/...`；配置文件使用 `~/.elysiaclaw-<profile>/logs/...`）
