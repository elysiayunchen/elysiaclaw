...
ElysiaClaw 的 Gateway 网关可以提供兼容 OpenResponses 的 `POST /v1/responses` 端点。
...
底层实现中，请求作为正常的 Gateway 网关智能体运行执行（与 `elysiaclaw agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway 网关一致。
...
- `model: "elysiaclaw:<agentId>"`（示例：`"elysiaclaw:main"`、`"elysiaclaw:beta"`）
- `model: "agent:<agentId>"`（别名）

或通过头指定特定的 ElysiaClaw 智能体：

- `x-elysiaclaw-agent-id: <agentId>`（默认：`main`）

高级：

- `x-elysiaclaw-session-key: <sessionKey>` 完全控制会话路由。
