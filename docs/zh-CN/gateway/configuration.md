...
- 当前工作目录中的 `.env`（如果存在）
- `~/.elysiaclaw/.env`（即 `$OPENCLAW_STATE_DIR/.env`）作为全局回退 `.env`
...
- `~/.elysiaclaw/credentials/oauth.json`（或 `$OPENCLAW_STATE_DIR/credentials/oauth.json`）
...
- 如果你希望 `models.json` 存储在其他位置，请使用 `OPENCLAW_AGENT_DIR`（或 `PI_CODING_AGENT_DIR`）覆盖智能体配置根目录（默认：`~/.elysiaclaw/agents/main/agent`）。
...
Gateway 网关监视 `~/.elysiaclaw/elysiaclaw.json`（或 `OPENCLAW_CONFIG_PATH`）并自动应用更改。
...
- `~/.elysiaclaw/elysiaclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
...
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `elysiaclaw`（通告 `elysiaclaw.local`）。通过 `OPENCLAW_MDNS_HOSTNAME` 覆盖。
