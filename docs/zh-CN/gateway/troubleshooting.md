...
read_when:
  - 调查运行时问题或故障
summary: ElysiaClaw 常见故障的快速故障排除指南
title: 故障排除
x-i18n:
...
| `elysiaclaw gateway probe`           | Gateway 网关发现 + 可达性（本地 + 远程目标）                                          | 当你怀疑正在探测错误的 Gateway 网关时 |
| `elysiaclaw channels status --probe` | 向运行中的 Gateway 网关查询渠道状态（并可选探测）                                     | 当 Gateway 网关可达但渠道异常时       |
...
```bash
elysiaclaw models auth paste-token --provider anthropic
elysiaclaw models status
```
...
- 运行向导并将 Gateway 网关运行模式设置为 **Local**：
  ```bash
  elysiaclaw configure
  ```
- 或直接设置：
  ```bash
  elysiaclaw config set gateway.mode local
  ```
...
- 设置远程 URL 并保持 `gateway.mode=remote`：
  ```bash
  elysiaclaw config set gateway.mode remote
  elysiaclaw config set gateway.remote.url "wss://gateway.example.com"
  ```

**仅临时/开发使用：** 传递 `--allow-unconfigured` 以在没有
`gateway.mode=local` 的情况下启动 Gateway 网关。

**还没有配置文件？** 运行 `elysiaclaw setup` 创建初始配置，然后重新运行
Gateway 网关。
...
变量如 `DISPLAY` 应该放在 `~/.elysiaclaw/.env` 中（由 Gateway 网关早期加载）。
...
服务是用 Bun 或版本管理的 Node 路径安装的，运行 `elysiaclaw doctor`
迁移到系统 Node 安装。
...
- 或将密钥烘焙到你的自定义沙箱镜像中
- 然后运行 `elysiaclaw sandbox recreate --agent <id>`（或 `--all`）
...
- `gateway.mode` 必须为 `local` 才能运行 `elysiaclaw gateway` 和服务。
...
**如果 `elysiaclaw gateway status` 显示配置不匹配**
...
- 修复：运行 `elysiaclaw doctor` 更新它（或 `elysiaclaw gateway install --force` 完全重写）。
...
如果你从旧版本升级，你的磁盘上可能仍有 `~/elysiaclaw`。
...
症状：`pwd` 或文件工具显示 `~/.elysiaclaw/sandboxes/...` 即使你
期望的是主机工作区。
...
ElysiaClaw 有意拒绝**较旧/不安全的模型**（尤其是那些更容易受到提示词注入攻击的模型）。如果你看到此错误，该模型名称已不再支持。

**修复：**

- 为提供商选择**最新**模型并更新你的配置或模型别名。
- 如果你不确定哪些模型可用，运行 `elysiaclaw models list` 或
  `elysiaclaw models scan` 并选择一个支持的模型。
...
# 或者如果你想快速过滤：
tail -f "$(ls -t /tmp/elysiaclaw/elysiaclaw-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
elysiaclaw logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.elysiaclaw/agents/<agentId>/sessions/
```
...
# 查看最近的连接事件
elysiaclaw logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/elysiaclaw/elysiaclaw-*.log | head -1)" | tail -20
```

### 高内存使用

ElysiaClaw 在内存中保留对话历史。
...
当配置包含未知键、格式错误的值或无效类型时，ElysiaClaw 现在拒绝启动。
这是为了安全而故意设计的。
...
```bash
elysiaclaw doctor
elysiaclaw doctor --fix
```

注意事项：

- `elysiaclaw doctor` 报告每个无效条目。
- `elysiaclaw doctor --fix` 应用迁移/修复并重写配置。
- 诊断命令如 `elysiaclaw logs`、`elysiaclaw health`、`elysiaclaw status`、`elysiaclaw gateway status` 和 `elysiaclaw gateway probe` 即使配置无效也能运行。

### "All models failed" — 我应该首先检查什么？

- **凭证**存在于正在尝试的提供商（认证配置文件 + 环境变量）。
- **模型路由**：确认 `agents.defaults.model.primary` 和回退是你可以访问的模型。
- `/tmp/elysiaclaw/…` 中的 **Gateway 网关日志**以获取确切的提供商错误。
- **模型状态**：使用 `/model status`（聊天）或 `elysiaclaw models status`（CLI）。
...
5. 运行 `elysiaclaw channels status --probe` 获取审计提示。
...
端点接受 JSON Schema 的严格子集。ElysiaClaw 在当前 `main` 中清理/规范化工具
...
如果重置不起作用，在 [`scripts/package-mac-app.sh`](https://github.com/elysiaclaw/elysiaclaw/blob/main/scripts/package-mac-app.sh) 中更改 `BUNDLE_ID`（例如，添加 `.test` 后缀）并重新构建。这会强制 macOS 将其视为新应用。
...
# 或：launchctl bootout gui/$UID/bot.molt.gateway（用 bot.molt.<profile> 替换；旧版 com.elysiaclaw.* 仍然有效）
```
...
**修复 3：检查 CLI 安装**
确保全局 `elysiaclaw` CLI 已安装且与应用版本匹配：

```bash
elysiaclaw --version
npm install -g elysiaclaw@<version>
```
...
elysiaclaw gateway --verbose
elysiaclaw channels login --verbose
```
...
elysiaclaw gateway status --deep

# Gateway 网关是否可达？
elysiaclaw health --json
# 如果失败，使用连接详情重新运行：
elysiaclaw health --verbose
...
elysiaclaw logs --follow
# 如果 RPC 宕机的备用方案
tail -20 /tmp/elysiaclaw/elysiaclaw-*.log
```
...
1. 首先检查日志：`/tmp/elysiaclaw/`（默认：`elysiaclaw-YYYY-MM-DD.log`，或你配置的 `logging.file`）
2. 在 GitHub 上搜索现有问题
3. 提交新问题时包含：
   - ElysiaClaw 版本
   - 相关日志片段
