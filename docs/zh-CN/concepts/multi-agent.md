...
- 配置：`~/.elysiaclaw/elysiaclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
- 状态目录：`~/.elysiaclaw`（或 `OPENCLAW_STATE_DIR`）
...
- 工作区默认为 `~/.elysiaclaw/workspace`（或当设置了 `OPENCLAW_PROFILE` 时为 `~/.elysiaclaw/workspace-<profile>`）。
