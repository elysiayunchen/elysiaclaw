...
summary: "Architecture of ElysiaClaw's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in ElysiaClaw
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how ElysiaClaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

ElysiaClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, ElysiaClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in ElysiaClaw's local TUI mode)                                           |
...
├── pi-tools.ts                    # createElysiaClawCodingTools()
...
├── elysiaclaw-tools.ts              # ElysiaClaw-specific tools
...
Image injection is prompt-local: ElysiaClaw loads image refs from the current prompt and
...
2. **Custom Replacements**: ElysiaClaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **ElysiaClaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures ElysiaClaw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, ElysiaClaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
ElysiaClaw wraps this with `guardSessionManager()` for tool result safety.
...
ElysiaClaw maintains an auth profile store with multiple API keys per provider:
...
ElysiaClaw loads custom pi extensions for specialized behavior:
...
ElysiaClaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | ElysiaClaw Embedded                                                                                |
| --------------- | ----------------------- | -------------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                     |
| Tools           | Default coding tools    | Custom ElysiaClaw tool suite                                                                       |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                        |
| Session storage | `~/.pi/agent/sessions/` | `~/.elysiaclaw/agents/<agentId>/sessions/` (or `$ELYSIACLAW_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `ELYSIACLAW_LIVE_TEST=1`)
