...
- `/usage cost` in chats: local cost summary aggregated from ElysiaClaw session logs.
- CLI: `elysiaclaw status --usage` prints a full per-provider breakdown.
- CLI: `elysiaclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
