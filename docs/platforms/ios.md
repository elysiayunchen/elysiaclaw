...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `elysiaclaw.internal.`), **or**
...
elysiaclaw gateway call node.list --params "{}"
```
...
- Only genuine ElysiaClaw iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight ElysiaClaw builds.
...
The Gateway advertises `_elysiaclaw-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `elysiaclaw.internal.`) and Tailscale split DNS.
...
```bash
elysiaclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__elysiaclaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__elysiaclaw__/canvas/` and `/__elysiaclaw__/a2ui/`.
...
```bash
elysiaclaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__elysiaclaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
elysiaclaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `elysiaclaw devices list` and approve manually.
