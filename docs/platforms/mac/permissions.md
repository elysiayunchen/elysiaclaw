...
- Same path: run the app from a fixed location (for ElysiaClaw, `dist/ElysiaClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.elysiaclaw.mac
sudo tccutil reset ScreenCapture ai.elysiaclaw.mac
...
Workaround: move files into the ElysiaClaw workspace (`~/.elysiaclaw/workspace`) if you want to avoid per-folder grants.
