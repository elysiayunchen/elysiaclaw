---
summary: "ElysiaClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up ElysiaClaw on a Raspberry Pi
  - Running ElysiaClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# ElysiaClaw on Raspberry Pi

## Goal

Run a persistent, always-on ElysiaClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u elysiaclaw -f
```

## 9) Access the ElysiaClaw Dashboard
...
```bash
ssh user@gateway-host 'elysiaclaw dashboard --no-open'
```
...
If this Pi is mostly running ElysiaClaw, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit elysiaclaw
```
...
If possible, keep ElysiaClaw state/cache on SSD-backed storage to avoid SD-card
...
Most ElysiaClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled elysiaclaw

# Enable if not
sudo systemctl enable elysiaclaw

# Start on boot
sudo systemctl start elysiaclaw
```
...
```bash
# Check logs
journalctl -u elysiaclaw --no-pager -n 100

# Common fix: rebuild
cd ~/elysiaclaw  # if using hackable install
npm run build
sudo systemctl restart elysiaclaw
```
