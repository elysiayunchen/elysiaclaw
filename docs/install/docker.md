---
summary: "Optional Docker-based setup and onboarding for ElysiaClaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run ElysiaClaw on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full ElysiaClaw in Docker)
...
- Need the URL again? Run `docker compose run --rm elysiaclaw-cli dashboard --no-open`.
...
```bash
docker compose run -T --rm elysiaclaw-cli gateway probe
docker compose run -T --rm elysiaclaw-cli devices list --json
```
...
`elysiaclaw-cli` uses `network_mode: "service:elysiaclaw-gateway"` so CLI commands can
...
network path instead of the bundled `elysiaclaw-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `elysiaclaw-cli`.

It writes config/workspace on the host:

- `~/.elysiaclaw/`
- `~/.elysiaclaw/workspace`
...
- [GitHub Container Registry package](https://github.com/elysiaclaw/elysiaclaw/pkgs/container/elysiaclaw)

Use image name `ghcr.io/elysiaclaw/elysiaclaw` (not similarly named Docker Hub
images).
...
- `org.opencontainers.image.source=https://github.com/elysiaclaw/elysiaclaw`
- `org.opencontainers.image.url=https://elysiaclaw.ai`
- `org.opencontainers.image.documentation=https://docs.elysiaclaw.ai/install/docker`
- `org.opencontainers.image.licenses=MIT`
- `org.opencontainers.image.title=ElysiaClaw`
- `org.opencontainers.image.description=ElysiaClaw gateway and CLI runtime container image`
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/elysiaclaw/elysiaclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
See [`ClawDock` Helper README](https://github.com/elysiaclaw/elysiaclaw/blob/main/scripts/shell-helpers/README.md) for details.

### Manual flow (compose)

```bash
docker build -t elysiaclaw:local -f Dockerfile .
docker compose run --rm elysiaclaw-cli onboard
docker compose up -d elysiaclaw-gateway
```
...
```bash
docker compose run --rm elysiaclaw-cli dashboard --no-open
docker compose run --rm elysiaclaw-cli devices list
docker compose run --rm elysiaclaw-cli devices approve <requestId>
```
...
`/home/node/.elysiaclaw`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/elysiaclaw-config /path/to/elysiaclaw-workspace
```
...
```bash
docker compose run --rm elysiaclaw-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm elysiaclaw-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm elysiaclaw-cli channels add --channel discord --token "<token>"
```
...
background. In plain terms: Docker keeps checking if ElysiaClaw is still
...
```bash
docker compose run --rm elysiaclaw-cli config set gateway.mode local
docker compose run --rm elysiaclaw-cli config set gateway.bind lan
docker compose run --rm elysiaclaw-cli devices list --url ws://127.0.0.1:18789
```
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.elysiaclaw/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  ElysiaClaw auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `elysiaclaw sandbox recreate ...` command.
...
This builds `elysiaclaw-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `elysiaclaw-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "elysiaclaw-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `elysiaclaw-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`elysiaclaw-sandbox-browser`) instead of global `bridge`.
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-elysiaclaw-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-elysiaclaw-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-elysiaclaw-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/elysiaclaw/elysiaclaw/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: ElysiaClaw runs commands with `sh -lc` (login shell), which
