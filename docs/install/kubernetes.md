---
summary: "Deploy ElysiaClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run ElysiaClaw on a Kubernetes cluster
  - You want to test ElysiaClaw in a Kubernetes environment
title: "Kubernetes"
---

# ElysiaClaw on Kubernetes

A minimal starting point for running ElysiaClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

ElysiaClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/elysiaclaw 18789:18789 -n elysiaclaw
...
├── ConfigMap/elysiaclaw-config  # elysiaclaw.json + AGENTS.md
└── Secret/elysiaclaw-secrets    # Gateway token + API keys
```
...
Edit `elysiaclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret elysiaclaw-secrets -n elysiaclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/elysiaclaw -n elysiaclaw
```
...
```yaml
image: ghcr.io/elysiaclaw/elysiaclaw:2026.3.1
```
...
└── manifests/
    ...
    ├── configmap.yaml          # elysiaclaw.json + AGENTS.md
