...
Deploy elysiaclaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/elysiaclaw/elysiaclaw)
...
repository configures everything needed to run elysiaclaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: elysiaclaw_STATE_DIR
        value: /data/.elysiaclaw
      - key: elysiaclaw_WORKSPACE_DIR
        value: /data/workspace
      - key: elysiaclaw_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: elysiaclaw-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/elysiaclaw`.
...
If you use the original elysiaclaw repository, Render will not auto-deploy your elysiaclaw. To update it, run a manual Blueprint sync from the dashboard.
...
For elysiaclaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any elysiaclaw host.
