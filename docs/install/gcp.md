---
summary: "Run ElysiaClaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want ElysiaClaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# ElysiaClaw on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent ElysiaClaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "ElysiaClaw 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the ElysiaClaw Gateway in Docker
- Persist `~/.elysiaclaw` + `~/.elysiaclaw/workspace` on the host (survives restarts/rebuilds)
...
```bash
gcloud projects create my-elysiaclaw-project --name="ElysiaClaw Gateway"
gcloud config set project my-elysiaclaw-project
```
...
```bash
gcloud compute instances create elysiaclaw-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh elysiaclaw-gateway --zone=us-central1-a
```
...
## 6) Clone the ElysiaClaw repository
...
ELYSIACLAW_CONFIG_DIR=/home/$USER/.elysiaclaw
ELYSIACLAW_WORKSPACE_DIR=/home/$USER/.elysiaclaw/workspace
...
When binding to LAN (`ELYSIACLAW_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

```bash
docker compose run --rm elysiaclaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
```
...
```bash
gcloud compute ssh elysiaclaw-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
```
...
```bash
# Stop the VM first
gcloud compute instances stop elysiaclaw-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type elysiaclaw-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start elysiaclaw-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create elysiaclaw-deploy \
     --display-name="ElysiaClaw Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-elysiaclaw-project \
     --member="serviceAccount:elysiaclaw-deploy@my-elysiaclaw-project.iam.gserviceaccount.com" \
