...
```bash
elysiaclaw plugins install @elysiaclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
elysiaclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "ElysiaClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `elysiaclaw pairing list nextcloud-talk`
  - `elysiaclaw pairing approve nextcloud-talk <CODE>`
