...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects ElysiaClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current ElysiaClaw releases, so no separate plugin install
is required.
...
```bash
elysiaclaw plugins install @elysiaclaw/feishu
```
...
If you just installed ElysiaClaw, run the wizard:
...
2. Configuring app credentials in ElysiaClaw
...
1. You already ran `elysiaclaw channels add` for Feishu
2. The gateway is running (`elysiaclaw gateway status`)
...
Edit `~/.elysiaclaw/elysiaclaw.json`:
...
```bash
elysiaclaw pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  elysiaclaw pairing list feishu
  elysiaclaw pairing approve feishu <CODE>
  ```
...
2. Run `elysiaclaw logs --follow` and look for `chat_id`
...
2. Run `elysiaclaw logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `elysiaclaw gateway status`  | Show gateway status           |
| `elysiaclaw gateway install` | Install/start gateway service |
| `elysiaclaw gateway stop`    | Stop gateway service          |
| `elysiaclaw gateway restart` | Restart gateway service       |
| `elysiaclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `elysiaclaw logs --follow`
...
5. Ensure the gateway is running: `elysiaclaw gateway status`
6. Check logs: `elysiaclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.elysiaclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.elysiaclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
