...
read_when:
  - Setting up Twitch chat integration for ElysiaClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. ElysiaClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
elysiaclaw plugins install @elysiaclaw/twitch
```

Local checkout (when running from a git repo):

```bash
elysiaclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ELYSIACLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "elysiaclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ELYSIACLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
ELYSIACLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
