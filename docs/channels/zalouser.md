...
read_when:
  - Setting up Zalo Personal for ElysiaClaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside ElysiaClaw.
...
- Install via CLI: `elysiaclaw plugins install @elysiaclaw/zalouser`
- Or from a source checkout: `elysiaclaw plugins install ./extensions/zalouser`
...
- `elysiaclaw pairing list zalouser`
- `elysiaclaw pairing approve zalouser <code>`
...
- On startup, ElysiaClaw resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, ElysiaClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in ElysiaClaw state. Example:
...
- ElysiaClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, ElysiaClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `elysiaclaw channels logout --channel zalouser && elysiaclaw channels login --channel zalouser`
...
- The channel now runs fully in ElysiaClaw without external CLI binaries.
