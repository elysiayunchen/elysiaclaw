...
ElysiaClaw routes replies **back to the channel where a message came from**. The
...
ElysiaClaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, ElysiaClaw still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when ElysiaClaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.elysiaclaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.elysiaclaw`):

- `~/.elysiaclaw/agents/<agentId>/sessions/sessions.json`
