...
read_when:
  ...
  - Reviewing ElysiaClaw security posture
title: "Pairing"
---

# Pairing

“Pairing” is ElysiaClaw’s explicit **owner approval** step.
...
Stored under `~/.elysiaclaw/credentials/`:
...
3. On your phone, open the ElysiaClaw iOS app → Settings → Gateway.
...
Stored under `~/.elysiaclaw/devices/`:
...
- The legacy `node.pair.*` API (CLI: `elysiaclaw nodes pending/approve`) is a
