...
Matrix is an open, decentralized messaging protocol. ElysiaClaw connects as a Matrix **user**
...
```bash
elysiaclaw plugins install @elysiaclaw/matrix
```

Local checkout (when running from a git repo):

```bash
elysiaclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `elysiaclaw plugins install @elysiaclaw/matrix`
   - From a local checkout: `elysiaclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: ElysiaClaw calls the same
     login endpoint, stores the access token in `~/.elysiaclaw/credentials/matrix/credentials.json`,
...
- On first connection, ElysiaClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  ElysiaClaw logs a warning.
...
`~/.elysiaclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `elysiaclaw pairing list matrix`
  - `elysiaclaw pairing approve matrix <CODE>`
...
- On startup, ElysiaClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
