...
<Tabs>
  <Tab title="Socket Mode (default)">
    <Steps>
      ...
      <Step title="Configure ElysiaClaw">
...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure ElysiaClaw HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `elysiaclaw pairing approve slack <code>`.
...
- The interactive callback values are ElysiaClaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, ElysiaClaw falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while ElysiaClaw is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "ElysiaClaw",
    "description": "Slack connector for ElysiaClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "ElysiaClaw",
      "always_online": false
    },
    "app_home": {
...
ElysiaClaw supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, ElysiaClaw falls back to normal delivery for remaining payloads.
