---
summary: "Mattermost bot setup and ElysiaClaw config"
read_when:
...
```bash
elysiaclaw plugins install @elysiaclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
elysiaclaw plugins install ./extensions/mattermost
```
...
4. Configure ElysiaClaw and start the gateway.
...
Native slash commands are opt-in. When enabled, ElysiaClaw registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, ElysiaClaw derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as ElysiaClaw.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to ElysiaClaw.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from ElysiaClaw, not `404`.
...
- Approve via:
  - `elysiaclaw pairing list mattermost`
  - `elysiaclaw pairing approve mattermost <CODE>`
...
Use these target formats with `elysiaclaw message send` or cron/webhooks:
...
ElysiaClaw resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), ElysiaClaw sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
- If `interactions.callbackBaseUrl` is omitted, ElysiaClaw derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and ElysiaClaw run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="elysiaclaw-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"elysiaclaw-mattermost-interactions",
...
`elysiaclaw message send` and cron/webhook deliveries.
