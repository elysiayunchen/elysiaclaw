...
Tlon is a decentralized messenger built on Urbit. ElysiaClaw connects to your Urbit ship and can
...
```bash
elysiaclaw plugins install @elysiaclaw/tlon
```

Local checkout (when running from a git repo):

```bash
elysiaclaw plugins install ./extensions/tlon
```
...
By default, ElysiaClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `elysiaclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, ElysiaClaw replies in-thread.
