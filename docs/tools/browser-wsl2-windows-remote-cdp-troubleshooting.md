...
read_when:
  - Running ElysiaClaw Gateway in WSL2 while Chrome lives on Windows
...
- ElysiaClaw Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/elysiaclaw/elysiaclaw/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome-relay` profile plus the ElysiaClaw Chrome extension.
...
- ElysiaClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, ElysiaClaw is not the problem yet.
...
Fix that before touching ElysiaClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point ElysiaClaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting ElysiaClaw to succeed
...
```bash
elysiaclaw browser open https://example.com --browser-profile remote
elysiaclaw browser tabs --browser-profile remote
```

For the extension relay:

```bash
elysiaclaw browser tabs --browser-profile chrome-relay
```

Good result:

- the tab opens in Windows Chrome
- `elysiaclaw browser tabs` returns the target
...
3. ElysiaClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug ElysiaClaw config or Control UI auth
