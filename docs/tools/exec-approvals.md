...
- **gateway host** → `elysiaclaw` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, ElysiaClaw also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.elysiaclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.elysiaclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `elysiaclaw approvals allowlist ...`).
- `elysiaclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `elysiaclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.elysiaclaw/exec-approvals.json` directly.

CLI: `elysiaclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, ElysiaClaw does not treat
...
topics, ElysiaClaw preserves the topic for the approval prompt and the post-approval follow-up.
