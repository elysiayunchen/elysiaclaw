...
- `commands.config` (default `false`) enables `/config` (reads/writes `elysiaclaw.json`).
...
- For full provider usage breakdown, use `elysiaclaw status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- In multi-account channels, config-targeted `/allowlist --account <id>` and `/config set channels.<provider>.accounts.<id>...` also honor the target account's `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from ElysiaClaw session logs.
...
```
/debug show
/debug set messages.responsePrefix="[elysiaclaw]"
...
- Overrides apply immediately to new config reads, but do **not** write to `elysiaclaw.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`elysiaclaw.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[elysiaclaw]"
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/elysiaclaw`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
