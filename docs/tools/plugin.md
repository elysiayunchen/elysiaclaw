---
summary: "ElysiaClaw plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends ElysiaClaw with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core ElysiaClaw yet (or you want to keep optional features out of your main
install).
...
```bash
elysiaclaw plugins list
```

2. Install an official plugin (example: Voice Call):

```bash
elysiaclaw plugins install @elysiaclaw/voice-call
```
...
those to a prerelease, ElysiaClaw stops and asks you to opt in explicitly with a
...
ElysiaClaw's plugin system has four layers:

1. **Manifest + discovery**
   ElysiaClaw finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads
   `elysiaclaw.plugin.json` plus package metadata first.
...
4. **Surface consumption**
   The rest of ElysiaClaw reads the registry to expose tools, channels, provider
...
That split lets ElysiaClaw validate config, explain missing/disabled plugins, and
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@elysiaclaw/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@elysiaclaw/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@elysiaclaw/zalouser`
- [Matrix](/channels/matrix) — `@elysiaclaw/matrix`
- [Nostr](/channels/nostr) — `@elysiaclaw/nostr`
- [Zalo](/channels/zalo) — `@elysiaclaw/zalo`
- [Microsoft Teams](/channels/msteams) — `@elysiaclaw/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

ElysiaClaw plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
At startup, ElysiaClaw does roughly this:

1. discover candidate plugin roots
2. read `elysiaclaw.plugin.json` and package metadata
...
The manifest is the control-plane source of truth. ElysiaClaw uses it to:
...
ElysiaClaw keeps short in-process caches for:
...
Use SDK subpaths instead of the monolithic `elysiaclaw/plugin-sdk` import when
authoring plugins:

- `elysiaclaw/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `elysiaclaw/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `elysiaclaw/plugin-sdk/telegram` for Telegram channel plugins.
- `elysiaclaw/plugin-sdk/discord` for Discord channel plugins.
- `elysiaclaw/plugin-sdk/slack` for Slack channel plugins.
- `elysiaclaw/plugin-sdk/signal` for Signal channel plugins.
- `elysiaclaw/plugin-sdk/imessage` for iMessage channel plugins.
- `elysiaclaw/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `elysiaclaw/plugin-sdk/line` for LINE channel plugins.
- `elysiaclaw/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `elysiaclaw/plugin-sdk/acpx`, `elysiaclaw/plugin-sdk/bluebubbles`,
  `elysiaclaw/plugin-sdk/copilot-proxy`, `elysiaclaw/plugin-sdk/device-pair`,
  `elysiaclaw/plugin-sdk/diagnostics-otel`, `elysiaclaw/plugin-sdk/diffs`,
  `elysiaclaw/plugin-sdk/feishu`,
  `elysiaclaw/plugin-sdk/google-gemini-cli-auth`, `elysiaclaw/plugin-sdk/googlechat`,
  `elysiaclaw/plugin-sdk/irc`, `elysiaclaw/plugin-sdk/llm-task`,
  `elysiaclaw/plugin-sdk/lobster`, `elysiaclaw/plugin-sdk/matrix`,
  `elysiaclaw/plugin-sdk/mattermost`, `elysiaclaw/plugin-sdk/memory-core`,
  `elysiaclaw/plugin-sdk/memory-lancedb`,
  `elysiaclaw/plugin-sdk/minimax-portal-auth`,
  `elysiaclaw/plugin-sdk/nextcloud-talk`, `elysiaclaw/plugin-sdk/nostr`,
  `elysiaclaw/plugin-sdk/open-prose`, `elysiaclaw/plugin-sdk/phone-control`,
  `elysiaclaw/plugin-sdk/qwen-portal-auth`, `elysiaclaw/plugin-sdk/synology-chat`,
  `elysiaclaw/plugin-sdk/talk-voice`, `elysiaclaw/plugin-sdk/test-utils`,
  `elysiaclaw/plugin-sdk/thread-ownership`, `elysiaclaw/plugin-sdk/tlon`,
  `elysiaclaw/plugin-sdk/twitch`, `elysiaclaw/plugin-sdk/voice-call`,
  `elysiaclaw/plugin-sdk/zalo`, and `elysiaclaw/plugin-sdk/zalouser`.

Compatibility note:

- `elysiaclaw/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `elysiaclaw status`, `elysiaclaw status --all`,
  `elysiaclaw channels status`, `elysiaclaw channels resolve`, and doctor/config
...
- `<workspace>/.elysiaclaw/extensions/*.ts`
- `<workspace>/.elysiaclaw/extensions/*/index.ts`

3. Global extensions

- `~/.elysiaclaw/extensions/*.ts`
- `~/.elysiaclaw/extensions/*/index.ts`

4. Bundled extensions (shipped with ElysiaClaw, mostly disabled by default)

- `<elysiaclaw>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `elysiaclaw plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, ElysiaClaw logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. ElysiaClaw blocks candidates when:
...
Each plugin must include a `elysiaclaw.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `elysiaclaw.extensions`:
...
Security guardrail: every `elysiaclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `elysiaclaw plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `elysiaclaw.channel` and
install hints via `elysiaclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@elysiaclaw/nextcloud-talk",
  "elysiaclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@elysiaclaw/nextcloud-talk",
...
ElysiaClaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.elysiaclaw/mpm/plugins.json`
...
If a plugin exports `id`, ElysiaClaw uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `elysiaclaw.plugin.json` (`configSchema`).
...
ElysiaClaw preserves config for disabled plugins so toggling them back on is not
destructive.
...
ElysiaClaw augments `uiHints` at runtime based on discovered plugins:
...
```bash
elysiaclaw plugins list
elysiaclaw plugins info <id>
elysiaclaw plugins install <path>                 # copy a local file/dir into ~/.elysiaclaw/extensions/<id>
elysiaclaw plugins install ./extensions/voice-call # relative path ok
elysiaclaw plugins install ./plugin.tgz           # install from a local tarball
elysiaclaw plugins install ./plugin.zip           # install from a local zip
elysiaclaw plugins install -l ./extensions/voice-call # link (no copy) for dev
elysiaclaw plugins install @elysiaclaw/voice-call # install from npm
elysiaclaw plugins install @elysiaclaw/voice-call --pin # store exact resolved name@version
elysiaclaw plugins update <id>
elysiaclaw plugins update --all
elysiaclaw plugins enable <id>
elysiaclaw plugins disable <id>
elysiaclaw plugins doctor
```

`plugins update` only works for npm installs tracked under `plugins.installs`.
If stored integrity metadata changes between updates, ElysiaClaw warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `elysiaclaw voicecall`).
...
- Plugin-managed hooks show up in `elysiaclaw hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `elysiaclaw hooks`; enable/disable the plugin instead.
...
- When disabled, ElysiaClaw blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
setup inside ElysiaClaw, surface provider setup in onboarding/model-pickers, and
...
2. **Non-interactive setup**
   `auth[].runNonInteractive(ctx)` handles `elysiaclaw onboard --non-interactive`
   without prompts. Use this when the provider needs custom headless setup
   beyond the built-in simple API-key paths.
3. **Wizard integration**
   `wizard.onboarding` adds an entry to `elysiaclaw onboard`.
...
- `configPatch`: optional `elysiaclaw.json` changes
...
explicit method or let ElysiaClaw synthesize per-method choices.

ElysiaClaw validates provider wizard metadata when the plugin registers:
...
- `elysiaclaw models auth login --provider <id> [--method <id>]`
- `elysiaclaw onboard`
...
- `wizard.onboarding` adds a provider choice to `elysiaclaw onboard`.
...
- `config`: The current ElysiaClaw config
...
- Main package: `elysiaclaw` (this repo)
- Plugins: separate npm packages under `@elysiaclaw/*` (example: `@elysiaclaw/voice-call`)

Publishing contract:

- Plugin `package.json` must include `elysiaclaw.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `elysiaclaw plugins install <npm-spec>` uses `npm pack`, extracts into `~/.elysiaclaw/extensions/<id>/`, and enables it in config.
...
- CLI: `elysiaclaw voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `elysiaclaw.extensions` points at the built entrypoint (`dist/index.js`).
