...
ElysiaClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, ElysiaClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `elysiaclaw configure --section web`. It stores the key in
`~/.elysiaclaw/elysiaclaw.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.elysiaclaw/.env` (or your service environment). See [Env vars](/help/faq#how-does-elysiaclaw-load-environment-variables).
