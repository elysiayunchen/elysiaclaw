...
- Publish as `elysiaclaw/plugin-sdk` (or export from core under `elysiaclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ElysiaClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ElysiaClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ElysiaClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ElysiaClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ElysiaClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ElysiaClawConfig): string;
  };
};
```
...
- Introduce `elysiaclaw/plugin-sdk`.
- Add `api.runtime` to `ElysiaClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `elysiaclawRuntime: ">=2026.2.0"`).
