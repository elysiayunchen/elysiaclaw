...
read_when:
  - Designing workspace memory (~/.elysiaclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep ElysiaClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.elysiaclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.elysiaclaw/workspace` as canonical human-readable memory.
...
```
~/.elysiaclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in ElysiaClaw**, but keep a separable core library.

### Why integrate into ElysiaClaw?

- ElysiaClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `elysiaclaw memory recall "…" --k 25 --since 30d`
  - `elysiaclaw memory reflect --since 7d`
...
Pragmatic take for `~/.elysiaclaw/workspace`:
