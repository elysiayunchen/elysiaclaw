...
This plan defines how ElysiaClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- ElysiaClaw core owns ACP control plane concerns
...
ElysiaClaw should not reimplement ACP transport internals in core.
ElysiaClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in ElysiaClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under ElysiaClaw state dir
...
Core (must be in ElysiaClaw core):
...
- ACP event stream is normalized to ElysiaClaw reply chunks
...
For the first production backend (`extensions/acpx`), ElysiaClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses ElysiaClaw `sessionKey`)
...
- ElysiaClaw consumes ndjson events from `acpx --format json --json-strict`
