...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ElysiaClaw
...
Running ElysiaClaw via Node with `tsx` fails at startup with:

```
[elysiaclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `elysiaclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch elysiaclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node elysiaclaw.mjs status` works on Node 25.
