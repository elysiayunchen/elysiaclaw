...
This is handy when you need to toggle obscure settings without editing `elysiaclaw.json`.
...
- **Global `--dev` (profile):** isolates state under `~/.elysiaclaw-dev` and
...
## Raw stream logging (ElysiaClaw)

ElysiaClaw can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.elysiaclaw/logs/raw-stream.jsonl
```
