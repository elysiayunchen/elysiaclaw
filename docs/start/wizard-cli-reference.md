...
read_when:
  - You need detailed behavior for elysiaclaw onboard
...
This page is the full reference for `elysiaclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.elysiaclaw/elysiaclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `elysiaclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `elysiaclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `elysiaclaw health`.
    - `elysiaclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.elysiaclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.elysiaclaw/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`elysiaclaw onboard`](/cli/onboard)
