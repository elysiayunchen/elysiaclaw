...
The onboarding wizard is the **recommended** way to set up ElysiaClaw on macOS,
...
`elysiaclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`elysiaclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.elysiaclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `elysiaclaw doctor` first.
</Note>
...
Use `elysiaclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.elysiaclaw/workspace-<agentId>`.
...
- CLI command reference: [`elysiaclaw onboard`](/cli/onboard)
