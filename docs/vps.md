---
summary: "VPS hosting hub for ElysiaClaw (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/elysiaclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `ELYSIACLAW_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for stable startup path:
  - `ELYSIACLAW_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/elysiaclaw-compile-cache`
