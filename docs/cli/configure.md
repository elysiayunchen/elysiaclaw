---
summary: "CLI reference for `elysiaclaw configure` (interactive configuration prompts)"
read_when:
...
Tip: `elysiaclaw config` without a subcommand opens the same wizard. Use
`elysiaclaw config get|set|unset` for non-interactive edits.
...
```bash
elysiaclaw configure
elysiaclaw configure --section model --section channels
```
