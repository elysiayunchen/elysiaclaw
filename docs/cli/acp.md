...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a ElysiaClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`elysiaclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the ElysiaClaw gateway or agent instead.                                                                                                                                   |
...
# Remote Gateway
elysiaclaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
elysiaclaw acp --url wss://gateway-host:18789 --token-file ~/.elysiaclaw/gateway.token

# Attach to an existing session key
elysiaclaw acp --session agent:main:main

# Attach by label (must already exist)
elysiaclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
elysiaclaw acp --session agent:main:main --reset-session
```
...
```bash
elysiaclaw acp client

# Point the spawned bridge at a remote Gateway
elysiaclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.elysiaclaw/gateway.token

# Override the server command (default: elysiaclaw)
elysiaclaw acp client --server "node" --server-args elysiaclaw.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a ElysiaClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `elysiaclaw acp` over stdio.

Example config (persisted):

```bash
elysiaclaw config set gateway.remote.url wss://gateway-host:18789
elysiaclaw config set gateway.remote.token <token>
```
...
```bash
elysiaclaw acp --session agent:main:main
elysiaclaw acp --session agent:design:main
elysiaclaw acp --session agent:qa:bug-123
```
...
ElysiaClaw bot over ACP, use `acpx` with its built-in `elysiaclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx elysiaclaw` at `elysiaclaw acp`.
3. Target the ElysiaClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default ElysiaClaw ACP session
acpx elysiaclaw exec "Summarize the active ElysiaClaw session state."

# Persistent named session for follow-up turns
acpx elysiaclaw sessions ensure --name codex-bridge
acpx elysiaclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my ElysiaClaw work agent for recent context relevant to this repo."
```

If you want `acpx elysiaclaw` to target a specific Gateway and session key every
time, override the `elysiaclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "elysiaclaw": {
      "command": "env ELYSIACLAW_HIDE_BANNER=1 ELYSIACLAW_SUPPRESS_NOTES=1 elysiaclaw acp --url ws://127.0.0.1:18789 --token-file ~/.elysiaclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local ElysiaClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env ELYSIACLAW_HIDE_BANNER=1 ELYSIACLAW_SUPPRESS_NOTES=1 node elysiaclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an ElysiaClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “ElysiaClaw ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`ELYSIACLAW_GATEWAY_TOKEN`, `ELYSIACLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`ELYSIACLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `ELYSIACLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `elysiaclaw acp client` sets `ELYSIACLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `elysiaclaw`).
