---
summary: "CLI reference for `elysiaclaw qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
elysiaclaw qr
elysiaclaw qr --setup-code-only
elysiaclaw qr --json
elysiaclaw qr --remote
elysiaclaw qr --url wss://gateway.example/ws
```
