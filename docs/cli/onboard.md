---
summary: "CLI reference for `elysiaclaw onboard` (interactive onboarding wizard)"
read_when:
...
```bash
elysiaclaw onboard
elysiaclaw onboard --flow quickstart
elysiaclaw onboard --flow manual
elysiaclaw onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`ELYSIACLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `elysiaclaw gateway run`.
...
- Fastest first chat: `elysiaclaw dashboard` (Control UI, no channel setup).
