...
read_when:
  - You want to place an outbound voice call from ElysiaClaw
...
Voice calls for ElysiaClaw via a plugin. Supports outbound notifications and
...
- Use `elysiaclaw voicecall ...` or the `voice_call` tool
...
```bash
elysiaclaw plugins install ./extensions/voice-call
