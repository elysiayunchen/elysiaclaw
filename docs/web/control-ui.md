...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/elysiaclaw`)
...
If the page fails to load, start the Gateway first: `elysiaclaw gateway`.
...
you revoke it with `elysiaclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.elysiaclaw/elysiaclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop elysiaclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. ElysiaClaw
...
ElysiaClaw **blocks** Control UI connections without device identity.
