...
```bash
elysiaclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `elysiaclaw tui --deliver`
...
- Local shell commands receive `ELYSIACLAW_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `ELYSIACLAW_THEME=light` before launching `elysiaclaw tui`.
- To force the original dark palette instead, set `ELYSIACLAW_THEME=dark`.
...
- Check the Gateway logs: `elysiaclaw logs --follow`.
- Confirm the agent can run: `elysiaclaw status` and `elysiaclaw models status`.
...
- No agents in picker: check `elysiaclaw agents list` and your routing config.
