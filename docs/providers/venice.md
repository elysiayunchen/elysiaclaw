---
summary: "Use Venice AI privacy-focused models in ElysiaClaw"
read_when:
  - You want privacy-focused inference in ElysiaClaw
...
```bash
elysiaclaw onboard --auth-choice venice-api-key
```
...
```bash
elysiaclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, ElysiaClaw shows all available Venice models. Pick based on your needs:
...
```bash
elysiaclaw models set venice/kimi-k2-5
elysiaclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
elysiaclaw models list | grep venice
```

## Configure via `elysiaclaw configure`

1. Run `elysiaclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ElysiaClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
elysiaclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
elysiaclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
elysiaclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
elysiaclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
elysiaclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `elysiaclaw models list` to see currently available models. Some models may be temporarily offline.
