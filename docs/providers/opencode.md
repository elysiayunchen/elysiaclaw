---
summary: "Use OpenCode Zen and Go catalogs with ElysiaClaw"
read_when:
...
OpenCode exposes two hosted catalogs in ElysiaClaw:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. ElysiaClaw keeps the runtime provider ids
...
elysiaclaw onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
elysiaclaw onboard --auth-choice opencode-go
elysiaclaw onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
