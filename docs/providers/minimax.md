---
summary: "Use MiniMax M2.5 in ElysiaClaw"
read_when:
  - You want MiniMax models in ElysiaClaw
...
```bash
elysiaclaw plugins enable minimax-portal-auth  # skip if already loaded.
elysiaclaw gateway restart  # restart if gateway is already running
elysiaclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/elysiaclaw/elysiaclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `elysiaclaw.json`:
...
- Use `elysiaclaw models list` and `elysiaclaw models set minimax/MiniMax-M2.5` to switch.
...
- Running `elysiaclaw configure` and selecting **MiniMax M2.5**, or
