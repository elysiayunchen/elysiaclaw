...
read_when:
  - Setting up ElysiaClaw for the first time
...
ElysiaClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.elysiaclaw/elysiaclaw.json`.

If the file is missing, ElysiaClaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `elysiaclaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    elysiaclaw onboard       # full setup wizard
    elysiaclaw configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.elysiaclaw/elysiaclaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
ElysiaClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`elysiaclaw doctor`, `elysiaclaw logs`, `elysiaclaw health`, `elysiaclaw status`)
- Run `elysiaclaw doctor` to see exact issues
- Run `elysiaclaw doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `elysiaclaw.json`.
    ...
    ```bash
    elysiaclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `ELYSIACLAW_APNS_RELAY_BASE_URL` and `ELYSIACLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `ELYSIACLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.elysiaclaw/elysiaclaw.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `elysiaclaw config set` for single keys.
    </Warning>
    ...
    elysiaclaw gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.elysiaclaw/workspace\" } } }",
      "baseHash": "<hash>",
...
ElysiaClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.elysiaclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, ElysiaClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `ELYSIACLAW_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${ELYSIACLAW_GATEWAY_TOKEN}" } },
