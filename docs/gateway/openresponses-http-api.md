...
ElysiaClaw’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`elysiaclaw agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "elysiaclaw:<agentId>"`, `model: "agent:<agentId>"`, or `x-elysiaclaw-agent-id`
- use `x-elysiaclaw-session-key` for explicit session routing
