...
- During heartbeat runs, ElysiaClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `elysiaclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, ElysiaClaw skips the heartbeat run entirely (no model call).
...
```bash
elysiaclaw system event --text "Check for urgent follow-ups" --mode now
```
