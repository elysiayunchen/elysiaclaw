...
description: "Complete field-by-field reference for ~/.elysiaclaw/elysiaclaw.json"
summary: "Complete reference for every ElysiaClaw config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.elysiaclaw/elysiaclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — ElysiaClaw uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.elysiaclaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `elysiaclaw doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `elysiaclaw doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["elysiaclaw-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-elysiaclaw",
          requireMention: false,
...
- ElysiaClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `elysiaclaw plugins install @elysiaclaw/mattermost`.
...
- `commands.callbackUrl` must resolve to the ElysiaClaw gateway endpoint and be reachable from the Mattermost server.
...
ElysiaClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `elysiaclaw channels add` (or channel onboarding) while still on a single-account top-level channel config, ElysiaClaw moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `elysiaclaw doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@elysiaclaw", "elysiaclaw"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@elysiaclaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `elysiaclaw.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, ElysiaClaw auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/elysiaclaw" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, ElysiaClaw assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.elysiaclaw/sandboxes",
        docker: {
          ...
          apparmorProfile: "elysiaclaw-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "elysiaclaw-sandbox-browser:bookworm-slim",
          network: "elysiaclaw-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.elysiaclaw/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `elysiaclaw.json`.
noVNC observer access uses VNC auth by default and ElysiaClaw emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `elysiaclaw-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `ELYSIACLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `ELYSIACLAW_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `ELYSIACLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.elysiaclaw/workspace",
        agentDir: "~/.elysiaclaw/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@elysiaclaw"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.elysiaclaw/workspace-home" },
      { id: "work", workspace: "~/.elysiaclaw/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, ElysiaClaw resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.elysiaclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.elysiaclaw/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.elysiaclaw/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.elysiaclaw/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, ElysiaClaw starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.elysiaclaw/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, ElysiaClaw treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:elysiaclaw` | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.elysiaclaw/attachments/<uuid>/` with a `.manifest.json`.
...
ElysiaClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.elysiaclaw/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `ELYSIACLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), ElysiaClaw forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `elysiaclaw onboard --auth-choice opencode-zen` or `elysiaclaw onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `elysiaclaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `elysiaclaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `elysiaclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `elysiaclaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `elysiaclaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.elysiaclaw/extensions`, `<workspace>/.elysiaclaw/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `elysiaclaw plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ELYSIACLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `ELYSIACLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `ELYSIACLAW_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `ELYSIACLAW_APNS_RELAY_BASE_URL` / `ELYSIACLAW_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `ELYSIACLAW_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
Convenience flags: `--dev` (uses `~/.elysiaclaw-dev` + port `19001`), `--profile <name>` (uses `~/.elysiaclaw-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.elysiaclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-elysiaclaw-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ELYSIACLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.elysiaclaw/workspace/canvas",
    liveReload: true,
    // enabled: false, // or ELYSIACLAW_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__elysiaclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__elysiaclaw__/a2ui/`
...
- Also serves A2UI at `/__elysiaclaw__/a2ui/`.
...
- Hostname defaults to `elysiaclaw`. Override with `ELYSIACLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.elysiaclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `elysiaclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.elysiaclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${ELYSIACLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `elysiaclaw.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.elysiaclaw/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/elysiaclaw-vault-resolver",
...
- Legacy OAuth imports from `~/.elysiaclaw/credentials/oauth.json`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/elysiaclaw/elysiaclaw.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/elysiaclaw/elysiaclaw-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one ElysiaClaw.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `ELYSIACLAW_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `elysiaclaw doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `elysiaclaw.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
