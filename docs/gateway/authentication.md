...
ElysiaClaw supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `elysiaclaw gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.elysiaclaw/.env` so the daemon can read it:

```bash
cat >> ~/.elysiaclaw/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `elysiaclaw onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.elysiaclaw/.env`, systemd/launchd).
...
```bash
elysiaclaw models auth paste-token --provider anthropic
elysiaclaw models auth paste-token --provider openrouter
```
...
```bash
elysiaclaw models status --check
```
...
- Priority order:
  - `ELYSIACLAW_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- ElysiaClaw retries with the next key only for rate-limit errors (for example
...
Run `elysiaclaw models status` to confirm which profile is expiring. If the profile
