...
- If you install plugins from npm (`elysiaclaw plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.elysiaclaw/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
...
Assume anything under `~/.elysiaclaw/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:
