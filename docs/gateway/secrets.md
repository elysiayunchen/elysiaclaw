...
ElysiaClaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `ELYSIACLAW_GATEWAY_TOKEN` (or `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, ElysiaClaw runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). ElysiaClaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/ElysiaClaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/elysiaclaw"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `elysiaclaw.json` refs)
...
When reload-time activation fails after a healthy state, ElysiaClaw enters degraded secrets state.
...
- Strict command paths (for example `elysiaclaw memory` remote-memory paths and `elysiaclaw qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `elysiaclaw status`, `elysiaclaw status --all`, `elysiaclaw channels status`, `elysiaclaw channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `elysiaclaw secrets reload`.
...
```bash
elysiaclaw secrets audit --check
elysiaclaw secrets configure
elysiaclaw secrets audit --check
```
...
- plaintext values at rest (`elysiaclaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `elysiaclaw.json` refs)
...
- lets you select supported secret-bearing fields in `elysiaclaw.json` plus `auth-profiles.json` for one agent scope
...
- `elysiaclaw secrets configure --providers-only`
- `elysiaclaw secrets configure --skip-provider-setup`
- `elysiaclaw secrets configure --agent <id>`
...
```bash
elysiaclaw secrets apply --from /tmp/elysiaclaw-secrets-plan.json
elysiaclaw secrets apply --from /tmp/elysiaclaw-secrets-plan.json --dry-run
```
...
ElysiaClaw intentionally does not write rollback backups containing historical plaintext secret values.
