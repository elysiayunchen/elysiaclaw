...
- `elysiaclaw gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `elysiaclaw doctor` reports no blocking config/service issues.
- `elysiaclaw channels status --probe` shows connected/ready channels.
...
elysiaclaw config get agents.defaults.models
```
...
elysiaclaw status
elysiaclaw channels status --probe
elysiaclaw pairing list --channel <channel> [--account <id>]
elysiaclaw config get channels
elysiaclaw logs --follow
```
...
elysiaclaw logs --follow
elysiaclaw doctor
elysiaclaw gateway status --json
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `elysiaclaw config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `elysiaclaw devices list` then `elysiaclaw devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `elysiaclaw configure`). If you are running ElysiaClaw via Podman using the dedicated `elysiaclaw` user, the config lives at `~elysiaclaw/.elysiaclaw/elysiaclaw.json`.
...
```bash
elysiaclaw cron status
elysiaclaw cron list
elysiaclaw cron runs --id <jobId> --limit 20
elysiaclaw system heartbeat last
elysiaclaw logs --follow
```
...
elysiaclaw approvals get --node <idOrNameOrIp>
elysiaclaw logs --follow
elysiaclaw status
```
...
elysiaclaw browser start --browser-profile elysiaclaw
...
```bash
elysiaclaw gateway status
elysiaclaw config get gateway.mode
elysiaclaw config get gateway.remote.url
elysiaclaw config get gateway.auth.mode
```
...
```bash
elysiaclaw config get gateway.bind
