...
read_when:
  - Running ElysiaClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of ElysiaClaw
...
- You run ElysiaClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. ElysiaClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. ElysiaClaw extracts the user identity from the configured header
...
- ElysiaClaw can stay on loopback HTTP behind the proxy.
...
If ElysiaClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://elysiaclaw.example.com
    to: http://elysiaclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
elysiaclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy elysiaclaw:18789 {
...
location / {
    ...
    proxy_pass http://elysiaclaw:18789;
...
`elysiaclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update ElysiaClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `elysiaclaw security audit` and review findings
