...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `elysiaclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".elysiaclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ELYSIACLAW_SKIP_PROVIDERS: "1",
          ELYSIACLAW_SKIP_GMAIL_WATCHER: "1",
          ELYSIACLAW_SKIP_CRON: "1",
          ELYSIACLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
          ELYSIACLAW_SKIP_CANVAS_HOST: "1",
          ELYSIACLAW_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
