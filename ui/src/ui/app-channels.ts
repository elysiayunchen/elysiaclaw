...
export async function handleWhatsAppStart(host: ElysiaClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ElysiaClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ElysiaClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ElysiaClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ElysiaClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ElysiaClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: ElysiaClawApp): string | null {
...
function buildGatewayHttpHeaders(host: ElysiaClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: ElysiaClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: ElysiaClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: ElysiaClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ElysiaClawApp) {
  const state = host.nostrProfileFormState;
