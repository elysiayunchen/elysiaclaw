...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://elysiaclaw.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://elysiaclaw.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://elysiaclaw.ai/abc-123", baseHref)).toBe(
      "blob:https://elysiaclaw.ai/abc-123",
    );
  });
...
describe("openExternalUrlSafe", () => {
  it("nulls opener when window.open returns a proxy-like object", () => {
    ...
    vi.stubGlobal("window", {
      location: { href: "https://elysiaclaw.ai/chat" },
      open: openMock,
