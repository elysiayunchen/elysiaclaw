  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __ELYSIACLAW_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__ELYSIACLAW_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__ELYSIACLAW_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /elysiaclaw/ ");

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/elysiaclaw"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/elysiaclaw/chat",
    });

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/elysiaclaw"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    sessionStorage.setItem("elysiaclaw.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "elysiaclaw.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/elysiaclaw",
    ...
    expect(JSON.parse(localStorage.getItem("elysiaclaw.control.settings.v1") ?? "{}")).toEqual({
      gatewayUrl: "wss://gateway.example:8443/elysiaclaw",
      sessionKey: "agent",
  ...
  it("does not reuse a session token for a different gatewayUrl", async () => {
    ...
    localStorage.setItem(
      "elysiaclaw.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://other-gateway.example:8443/elysiaclaw",
        sessionKey: "main",
        lastActiveSessionKey: "main",
        theme: "claw",
        themeMode: "system",
