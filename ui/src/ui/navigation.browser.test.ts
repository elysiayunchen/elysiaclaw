...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/elysiaclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/elysiaclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/elysiaclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ELYSIACLAW_CONTROL_UI_BASE_PATH__ = "/elysiaclaw";
    const app = mountApp("/elysiaclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/elysiaclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/elysiaclaw/sessions");
  });
  ...
  it("strips query token params without importing them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("elysiaclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "elysiaclaw.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/elysiaclaw" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;
    ...
    expect(
      JSON.parse(localStorage.getItem("elysiaclaw.control.settings.v1") ?? "{}"),
    ).toMatchObject({
      gatewayUrl: "wss://gateway.example/elysiaclaw",
    });
    expect(JSON.parse(localStorage.getItem("elysiaclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/elysiaclaw";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/elysiaclaw");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/elysiaclaw#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/elysiaclaw");
    expect(app.settings.token).toBe("");
