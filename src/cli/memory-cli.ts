    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["elysiaclaw memory status", "Show index and provider status."],
          ["elysiaclaw memory status --deep", "Probe embedding provider readiness."],
          ["elysiaclaw memory index --force", "Force a full reindex."],
          ['elysiaclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'elysiaclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["elysiaclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.elysiaclaw.ai/cli/memory")}\n`,
    );
