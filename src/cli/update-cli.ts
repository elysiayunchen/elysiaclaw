...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update ElysiaClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["elysiaclaw update", "Update a source checkout (git)"],
        ["elysiaclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["elysiaclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["elysiaclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["elysiaclaw update --dry-run", "Preview actions without changing anything"],
        ["elysiaclaw update --no-restart", "Update without restarting the service"],
...
${theme.heading("Switch channels:")}
  ...
  - Run elysiaclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.elysiaclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["elysiaclaw update status", "Show channel + version status."],
          ["elysiaclaw update status --json", "JSON output."],
          ["elysiaclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.elysiaclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
