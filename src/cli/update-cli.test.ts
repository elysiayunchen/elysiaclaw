...
import type { ElysiaClawConfig, ConfigFileSnapshot } from "../config/types.elysiaclaw.js";
...
describe("update-cli", () => {
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveElysiaClawPackageRoot).mockResolvedValue(root);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveElysiaClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("elysiaclaw");
    ...
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "elysiaclaw-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("ElysiaClaw update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as ElysiaClawConfig,
        });
      },
  ...
  it("updateCommand reuses the captured invocation cwd when process.cwd later fails", async () => {
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        cwd: root,
        env: expect.objectContaining({
          ELYSIACLAW_STATE_DIR: path.resolve(originalCwd, "./state"),
        }),
        timeoutMs: 60_000,
      }),
    );
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ ELYSIACLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.ELYSIACLAW_UPDATE_IN_PROGRESS).toBeUndefined();
