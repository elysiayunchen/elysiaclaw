...
${formatHelpExamples([
  ['elysiaclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['elysiaclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'elysiaclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'elysiaclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['elysiaclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'elysiaclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.elysiaclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.elysiaclaw.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'elysiaclaw agents set-identity --agent main --name "ElysiaClaw" --emoji "🌸"',
    "Set name + emoji.",
  ],
  [
  ...
  [
    "elysiaclaw agents set-identity --identity-file ~/.elysiaclaw/workspace/IDENTITY.md --agent main",
