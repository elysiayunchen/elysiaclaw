    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["elysiaclaw nodes status", "List known nodes with live status."],
          ["elysiaclaw nodes pairing pending", "Show pending node pairing requests."],
          ['elysiaclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["elysiaclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.elysiaclaw.ai/cli/nodes")}\n`,
    );
