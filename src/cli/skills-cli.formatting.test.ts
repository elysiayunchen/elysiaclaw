...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "elysiaclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "elysiaclaw-bundled-skills-test-"));
    process.env.ELYSIACLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
