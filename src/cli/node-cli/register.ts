    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "elysiaclaw node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["elysiaclaw node status", "Check node host service status."],
          ["elysiaclaw node install", "Install the node host service."],
          ["elysiaclaw node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.elysiaclaw.ai/cli/node")}\n`,
    );
