...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["elysiaclaw security audit", "Run a local security audit."],
          ["elysiaclaw security audit --deep", "Include best-effort live Gateway probe checks."],
          ["elysiaclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["elysiaclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.elysiaclaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("ElysiaClaw security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("elysiaclaw security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("elysiaclaw security audit --fix")}`));
        if (!fixResult) {
