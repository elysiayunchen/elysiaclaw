  ...
  it("treats a gateway listener child pid as healthy ownership", async () => {
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 7001, ppid: 7000, commandLine: "elysiaclaw-gateway" }],
        hints: [],
      },
    });
  ...
  it("marks non-owned gateway listener pids as stale while runtime is running", async () => {
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "elysiaclaw-gateway" }],
        hints: [],
      },
    });
  ...
  it("treats a busy port as healthy when runtime status lags but the probe succeeds", async () => {
    ...
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 9100, commandLine: "elysiaclaw-gateway" }],
        hints: [],
      },
    });
