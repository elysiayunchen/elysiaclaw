...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.elysiaclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + ElysiaClaw hooks")
    ...
    .option("--hook-url <url>", "ElysiaClaw hook URL")
    .option("--hook-token <token>", "ElysiaClaw hook token")
