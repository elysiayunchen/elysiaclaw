...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub ELYSIACLAW_STATE_DIR.
    // Also keep it outside the ElysiaClaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["ELYSIACLAW_STATE_DIR"]);
    process.env.ELYSIACLAW_STATE_DIR = path.join(
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredElysiaClawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default ElysiaClaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default ElysiaClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
