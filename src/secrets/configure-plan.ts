...
export type ConfigureCandidate = {
  ...
  pathSegments: string[];
  label: string;
  configFile: "elysiaclaw.json" | "auth-profiles.json";
...
function getSecretProviders(config: ElysiaClawConfig): Record<string, SecretProviderConfig> {
...
export function buildConfigureCandidates(config: ElysiaClawConfig): ConfigureCandidate[] {
...
function configureCandidateSortKey(candidate: ConfigureCandidate): string {
  ...
  return `elysiaclaw:${candidate.path}`;
}
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredElysiaClawConfig?: ElysiaClawConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredElysiaClawConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const __elysiaclawCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "elysiaclaw.json" as const,
  ...
  return [...elysiaclawCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "elysiaclaw secrets configure",
