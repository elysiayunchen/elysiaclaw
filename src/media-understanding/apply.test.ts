...
const TEMP_MEDIA_PREFIX = "elysiaclaw-media-";
...
function createGroqAudioConfig(): ElysiaClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): ElysiaClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): ElysiaClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: ElysiaClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredElysiaClawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        ELYSIACLAW_AGENT_DIR: isolatedAgentDir,
