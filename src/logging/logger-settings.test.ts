...
let originalElysiaClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.ELYSIACLAW_TEST_FILE_LOG;
  originalElysiaClawLogLevel = process.env.ELYSIACLAW_LOG_LEVEL;
  delete process.env.ELYSIACLAW_TEST_FILE_LOG;
  delete process.env.ELYSIACLAW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.ELYSIACLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalElysiaClawLogLevel === undefined) {
    delete process.env.ELYSIACLAW_LOG_LEVEL;
  } else {
    process.env.ELYSIACLAW_LOG_LEVEL = originalElysiaClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
