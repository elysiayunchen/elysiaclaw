...
function telegramCfg(): ElysiaClawConfig {
  return { channels: { telegram: { botToken: "tok" } } } as ElysiaClawConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: ElysiaClawConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as ElysiaClawConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as ElysiaClawConfig;
...
function createSignalAccountOverrideCfg(): ElysiaClawConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as ElysiaClawConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as ElysiaClawConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as ElysiaClawConfig,
        accountId: undefined,
        params: {
