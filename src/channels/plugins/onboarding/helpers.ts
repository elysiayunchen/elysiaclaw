...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: ElysiaClawConfig) => Array<string | number> | undefined;
}): ElysiaClawConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: ElysiaClawConfig) => ElysiaClawConfig | Promise<ElysiaClawConfig>;
  applySet?: (
    cfg: ElysiaClawConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => ElysiaClawConfig | Promise<ElysiaClawConfig>;
}): Promise<{
  cfg: ElysiaClawConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in ElysiaClaw config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "ELYSIACLAW_SECRET",
      envVarFormatError:
        'Use an env var name like "ELYSIACLAW_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
