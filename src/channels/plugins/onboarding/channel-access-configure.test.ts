...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: ElysiaClawConfig, policy: ChannelAccessPolicy) => ElysiaClawConfig;
  resolveAllowlist: (params: { cfg: ElysiaClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: ElysiaClawConfig; resolved: TResolved }) => ElysiaClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: ElysiaClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: ElysiaClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: ElysiaClawConfig, policy: ChannelAccessPolicy): ElysiaClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: ElysiaClawConfig, policy: ChannelAccessPolicy): ElysiaClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: ElysiaClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: ElysiaClawConfig; resolved: string[] }) => {
      calls.push("apply");
