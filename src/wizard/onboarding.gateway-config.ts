...
import type { GatewayBindMode, GatewayTailscaleMode, ElysiaClawConfig } from "../config/config.js";
    ...
    if (tokenMode === "ref") {
      if (flow === "quickstart" && quickstartTokenRef) {
        ...
        const resolved = await promptSecretRefForOnboarding({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "ELYSIACLAW_GATEWAY_TOKEN",
          copy: {
            sourceMessage: "Where is this gateway token stored?",
            envVarPlaceholder: "ELYSIACLAW_GATEWAY_TOKEN",
          },
        });
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.ELYSIACLAW_GATEWAY_TOKEN)) ||
        randomToken();
      ...
      const tokenInput = await prompter.text({
        ...
        initialValue:
          ...
          normalizeGatewayTokenInput(process.env.ELYSIACLAW_GATEWAY_TOKEN) ??
          "",
      });
    ...
    if (!password) {
      const selectedMode = await resolveSecretInputModeForEnvSelection({
        ...
        copy: {
          ...
          plaintextHint: "Stores the password directly in ElysiaClaw config",
        },
      });
      if (selectedMode === "ref") {
        const resolved = await promptSecretRefForOnboarding({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "ELYSIACLAW_GATEWAY_PASSWORD",
          copy: {
            sourceMessage: "Where is this gateway password stored?",
            envVarPlaceholder: "ELYSIACLAW_GATEWAY_PASSWORD",
          },
        });
