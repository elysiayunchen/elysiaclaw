...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredElysiaClawTmpDir: vi.fn(() => "/tmp/elysiaclaw"),
}));
vi.mock("../infra/tmp-elysiaclaw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredElysiaClawTmpDir.mockReturnValue("/tmp/elysiaclaw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".elysiaclaw-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredElysiaClawTmpDir.mockReturnValue("/tmp/elysiaclaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "elysiaclaw-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "elysiaclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredElysiaClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "elysiaclaw-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "elysiaclaw-preferred", "downloads")}${path.sep}`),
    );
  });
