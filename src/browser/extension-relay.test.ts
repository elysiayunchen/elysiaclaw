...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "ELYSIACLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS",
      "ELYSIACLAW_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
    ]);
    process.env.ELYSIACLAW_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
    delete process.env.ELYSIACLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS;
    delete process.env.ELYSIACLAW_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-elysiaclaw-relay-token");
    expect(headers["x-elysiaclaw-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("allows CORS preflight from chrome-extension origins", async () => {
    ...
    const res = await fetch(`${sharedUrl}/json/version`, {
      ...
      headers: {
        ...
        "Access-Control-Request-Headers": "x-elysiaclaw-relay-token",
      },
    });
  ...
  it("closes CDP clients after reconnect grace when extension stays disconnected", async () => {
    process.env.ELYSIACLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS = "150";
  ...
  it("stops advertising websocket endpoint after reconnect grace expires", async () => {
    process.env.ELYSIACLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS = "120";
  ...
  it("accepts /json endpoints with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(sharedUrl)["x-elysiaclaw-relay-token"];
    expect(token).toBeTruthy();
  ...
  it("accepts raw gateway token for relay auth compatibility", async () => {
    ...
    const versionRes = await fetch(`${sharedUrl}/json/version`, {
      headers: { "x-elysiaclaw-relay-token": TEST_GATEWAY_TOKEN },
    });
    expect(versionRes.status).toBe(200);
  ...
  it(
    ...
    async () => {
      process.env.ELYSIACLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS = "200";
  ...
  it(
    ...
    async () => {
      process.env.ELYSIACLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS = "2000";
  ...
  it("does not swallow EADDRINUSE when occupied port is not an elysiaclaw relay", async () => {
    const port = await getFreePort();
