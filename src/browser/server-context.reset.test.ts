...
function localElysiaClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "elysiaclaw",
...
function createLocalElysiaClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localElysiaClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveElysiaClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    ...
    const ops = createLocalElysiaClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveElysiaClawUserDataDir: () => profileDir,
    });
