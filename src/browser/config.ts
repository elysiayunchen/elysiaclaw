import type { BrowserConfig, BrowserProfileConfig, ElysiaClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_ELYSIACLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_ELYSIACLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "elysiaclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ELYSIACLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ELYSIACLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an ElysiaClaw browser profile (routing config), not a Chrome user profile.
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ElysiaClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ELYSIACLAW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_ELYSIACLAW_BROWSER_PROFILE_NAME]
        ? DEFAULT_ELYSIACLAW_BROWSER_PROFILE_NAME
        : "user");
