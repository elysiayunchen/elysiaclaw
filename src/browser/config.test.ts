...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and sakura-pink color", () => {
    ...
    expect(profile?.name).toBe("elysiaclaw");
    expect(profile?.driver).toBe("elysiaclaw");
  ...
  it("derives default ports from ELYSIACLAW_GATEWAY_PORT when unset", () => {
    withEnv({ ELYSIACLAW_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ ELYSIACLAW_GATEWAY_PORT: undefined }, () => {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "elysiaclaw");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "elysiaclaw")!;
