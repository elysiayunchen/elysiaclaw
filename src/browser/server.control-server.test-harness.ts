...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "elysiaclaw" | "extension" | "existing-session";
      attachOnly?: boolean;
    }
  >;
...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.ELYSIACLAW_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "elysiaclaw",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/elysiaclaw" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    elysiaclaw: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchElysiaClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "elysiaclaw";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.ELYSIACLAW_GATEWAY_PORT;
  process.env.ELYSIACLAW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.ELYSIACLAW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.ELYSIACLAW_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.ELYSIACLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.ELYSIACLAW_GATEWAY_PASSWORD;
  } else {
    process.env.ELYSIACLAW_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
