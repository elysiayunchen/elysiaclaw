...
const { stopElysiaClawChromeMock, stopChromeExtensionRelayServerMock } = vi.hoisted(() => ({
  stopElysiaClawChromeMock: vi.fn(async () => {}),
...
vi.mock("./chrome.js", () => ({
  stopElysiaClawChrome: stopElysiaClawChromeMock,
}));
...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("starts relay only for extension profiles", async () => {
    resolveProfileMock.mockImplementation((_resolved: unknown, name: string) => {
      ...
      return { driver: "elysiaclaw", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopElysiaClawChromeMock.mockClear();
  ...
  it("stops all known profiles and ignores per-profile failures", async () => {
    ...
    expect(stopMap.elysiaclaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopElysiaClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
