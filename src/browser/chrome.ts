...
export async function launchElysiaClawChrome(
  ...
  const userDataDir = resolveElysiaClawUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_ELYSIACLAW_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateElysiaClawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  log.info(
    `🌸 elysiaclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopElysiaClawChrome(
