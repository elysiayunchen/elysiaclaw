...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".elysiaclaw-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + sakura-pink). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateElysiaClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ELYSIACLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ELYSIACLAW_BROWSER_COLOR).toUpperCase();
