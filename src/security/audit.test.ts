...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: ElysiaClawConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: ElysiaClawConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: ElysiaClawConfig) => string[];
  isConfigured?: (account: unknown, cfg: ElysiaClawConfig) => boolean;
  isEnabled?: (account: unknown, cfg: ElysiaClawConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ ELYSIACLAW_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        elysiaclaw: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.ELYSIACLAW_GATEWAY_TOKEN;
    const prevPassword = process.env.ELYSIACLAW_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.ELYSIACLAW_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.ELYSIACLAW_GATEWAY_PASSWORD;
      } else {
        process.env.ELYSIACLAW_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
      ...
      gateway: {
        ...
        auth: {
          password: {
            source: "env",
            provider: "default",
            id: "ELYSIACLAW_GATEWAY_PASSWORD",
          },
        },
      },
    };
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/elysiaclaw-safe-bins"];
    const cfg: ElysiaClawConfig = {
      tools: {
        exec: {
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("elysiaclaw-sbx-browser-old\nelysiaclaw-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "elysiaclaw-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "elysiaclaw-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("elysiaclaw-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("elysiaclaw-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "elysiaclaw-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "elysiaclaw-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-elysiaclaw.json");
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: ElysiaClawConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: ElysiaClawConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): ElysiaClawConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: ElysiaClawConfig = { session: { dmScope: "main" } };
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      const sourceConfig: ElysiaClawConfig = {
        channels: {
          discord: {
            enabled: true,
      ...
      const resolvedConfig: ElysiaClawConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.elysiaclaw/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: ElysiaClawConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.ELYSIACLAW_GATEWAY_TOKEN;
    process.env.ELYSIACLAW_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: ElysiaClawConfig = {
  ...
  it("scores unrestricted hooks.allowedAgentIds by gateway exposure", async () => {
    ...
    } satisfies NonNullable<ElysiaClawConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: ElysiaClawConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.elysiaclaw",
      configPath: "/Users/test/Dropbox/.elysiaclaw/elysiaclaw.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: ElysiaClawConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "elysiaclaw.json"),
        execDockerRawFn: execDockerRawUnavailable,
      });
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "elysiaclaw.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
    ...
    const cfg: ElysiaClawConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@elysiaclaw/voice-call@1.2.3",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@elysiaclaw/test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      path.join(pluginDir, "package.json"),
      JSON.stringify({ name: "@elysiaclaw/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@elysiaclaw/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        elysiaclaw: { extensions: ["../outside.js"] },
      }),
    );
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          elysiaclaw: { extensions: ["index.js"] },
        }),
      );
      await fs.writeFile(path.join(pluginDir, "index.js"), "export {};");
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.ELYSIACLAW_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.ELYSIACLAW_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
