...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        ELYSIACLAW_BASE_ENV: "base",
        ELYSIACLAW_TO_REMOVE: undefined,
      },
      env: {
        ELYSIACLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.ELYSIACLAW_BASE_ENV).toBe("base");
    expect(resolved.ELYSIACLAW_TEST_ENV).toBe("ok");
    expect(resolved.ELYSIACLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.ELYSIACLAW_CLI).toBe(ELYSIACLAW_CLI_ENV_VALUE);
  });
