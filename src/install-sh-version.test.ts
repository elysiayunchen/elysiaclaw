...
resolve_openclaw_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_ELYSIACLAW_BIN: cliPath,
        ELYSIACLAW_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("ElysiaClaw dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("ElysiaClaw dev's build");
    },
  );
