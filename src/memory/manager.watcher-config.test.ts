...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): ElysiaClawConfig {
    const defaults: NonNullable<NonNullable<ElysiaClawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: ElysiaClawConfig) {
