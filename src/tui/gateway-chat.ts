...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set ELYSIACLAW_GATEWAY_TOKEN/ELYSIACLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "elysiaclaw-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.ELYSIACLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.ELYSIACLAW_GATEWAY_PASSWORD);
