...
describe("canvas host state dir defaults", () => {
  it("uses ELYSIACLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("elysiaclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("ElysiaClaw Canvas");
      } finally {
        await handler.close();
      }
    });
