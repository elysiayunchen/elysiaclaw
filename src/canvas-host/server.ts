...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.elysiaclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.elysiaclawCanvasA2UIAction &&
        typeof window.elysiaclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.elysiaclawSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("elysiaclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android ElysiaClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.elysiaclawSendUserAction === "function"
        ? window.elysiaclawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ELYSIACLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.ELYSIACLAW_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>ElysiaClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
