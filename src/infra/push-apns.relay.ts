...
const GATEWAY_DEVICE_ID_HEADER = "x-elysiaclaw-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-elysiaclaw-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-elysiaclaw-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.ELYSIACLAW_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "ELYSIACLAW_APNS_RELAY_BASE_URL"
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.ELYSIACLAW_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require ELYSIACLAW_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.ELYSIACLAW_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
