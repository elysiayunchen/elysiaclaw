...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __elysiaclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__elysiaclawDiagnosticEventsState) {
    globalStore.__elysiaclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__elysiaclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: ElysiaClawConfig): boolean {
