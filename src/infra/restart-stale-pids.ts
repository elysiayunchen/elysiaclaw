 ...
 * returning control to the caller (typically `triggerElysiaClawRestart` →
...
/**
 * Parse elysiaclaw gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("elysiaclaw")) {
 ...
 * Returns only PIDs that belong to elysiaclaw gateway processes (not the current process).
 */
