...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers ELYSIACLAW_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for ELYSIACLAW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { ELYSIACLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when ELYSIACLAW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ ELYSIACLAW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { ELYSIACLAW_HOME: "/srv/elysiaclaw-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/elysiaclaw-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/elysiaclaw-home " },
      expected: "/srv/elysiaclaw-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/elysiaclaw-home/docs"));
  });
