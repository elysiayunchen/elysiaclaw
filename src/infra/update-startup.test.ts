...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ELYSIACLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.ELYSIACLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveElysiaClawPackageRoot } = await import("./elysiaclaw-root.js"));
    ...
    vi.mocked(resolveElysiaClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveElysiaClawPackageRoot).mockResolvedValue("/opt/elysiaclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/elysiaclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
