...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.ELYSIACLAW_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for ElysiaClaw. You can keep running the web gateway without it: \`${formatCliCommand("elysiaclaw gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
