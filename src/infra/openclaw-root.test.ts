...
describe("resolveElysiaClawPackageRoot", () => {
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    expect(resolveElysiaClawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("falls through from a non-openclaw moduleUrl candidate to cwd", async () => {
    ...
    expect(resolveElysiaClawPackageRootSync({ moduleUrl, cwd: cwdPkgRoot })).toBe(cwdPkgRoot);
    await expect(resolveElysiaClawPackageRoot({ moduleUrl, cwd: cwdPkgRoot })).resolves.toBe(
      cwdPkgRoot,
    );
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveElysiaClawPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveElysiaClawPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveElysiaClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveElysiaClawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
