...
export function triggerElysiaClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.ELYSIACLAW_SYSTEMD_UNIT,
      process.env.ELYSIACLAW_PROFILE,
    );
  ...
  const label =
    process.env.ELYSIACLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ELYSIACLAW_PROFILE);
