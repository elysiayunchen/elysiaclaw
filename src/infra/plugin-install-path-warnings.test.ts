...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/elysiaclaw-matrix-missing",
        installPath: "/tmp/elysiaclaw-matrix-missing",
      },
    });
    ...
    expect(issue).toEqual({
      kind: "missing-path",
      pluginId: "matrix",
      path: "/tmp/elysiaclaw-matrix-missing",
    });
    expect(
      formatPluginInstallPathIssue({
        issue: issue!,
        pluginLabel: "Matrix",
        defaultInstallCommand: "elysiaclaw plugins install @elysiaclaw/matrix",
        repoInstallCommand: "elysiaclaw plugins install ./extensions/matrix",
      }),
    ).toEqual([
      "Matrix is installed from a custom path that no longer exists: /tmp/elysiaclaw-matrix-missing",
      'Reinstall with "elysiaclaw plugins install @elysiaclaw/matrix".',
      'If you are running from a repo checkout, you can also use "elysiaclaw plugins install ./extensions/matrix".',
    ]);
  });
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<elysiaclaw plugins install @elysiaclaw/matrix>" when you want to return to the standard Matrix plugin.',
      'If you are intentionally running from a repo checkout, reinstall that checkout explicitly with "<elysiaclaw plugins install ./extensions/matrix>" after updates.',
    ]);
  });
});
