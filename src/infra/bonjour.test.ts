...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.ELYSIACLAW_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("elysiaclaw-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("elysiaclaw (ElysiaClaw)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("elysiaclaw");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("elysiaclaw.local");
