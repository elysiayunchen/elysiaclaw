...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@elysiaclaw/matrix", expected: "matrix" },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      resolveSafeInstallDir({
        baseDir: "/tmp/plugins",
        id: "@elysiaclaw/matrix",
        invalidNameMessage: "invalid plugin name",
      }),
    ).toEqual({
      ok: true,
      path: path.join("/tmp/plugins", "@elysiaclaw__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-install-safe-"));
    try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const outsideDir = await fs.mkdtemp(
        path.join(os.tmpdir(), "elysiaclaw-install-safe-outside-"),
      );
      try {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked base directories", async () => {
    const parentDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-install-safe-"));
