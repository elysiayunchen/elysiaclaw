...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ELYSIACLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*elysiaclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "elysiaclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "elysiaclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "elysiaclaw-gw");
  ...
  records.push(`_elysiaclaw-gw._tcp IN PTR ${instanceLabel}._elysiaclaw-gw._tcp`);
  records.push(`${instanceLabel}._elysiaclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._elysiaclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; elysiaclaw-content-hash: ${contentHash}\n${contentBody}`;
}
