...
const telegramChunkConfig: ElysiaClawConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes ElysiaClaw tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredElysiaClawTmpDir()]),
      }),
    );
  });

  it("includes ElysiaClaw tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes ElysiaClaw tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes ElysiaClaw tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: ElysiaClawConfig = { channels: { signal: { mediaMaxMb: 2 } } };
