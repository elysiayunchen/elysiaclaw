...
const mocks = vi.hoisted(() => ({
  ...
  loadElysiaClawPlugins: vi.fn(),
}));

const TEST_WORKSPACE_ROOT = "/tmp/elysiaclaw-test-workspace";
...
vi.mock("../../plugins/loader.js", () => ({
  loadElysiaClawPlugins: mocks.loadElysiaClawPlugins,
}));
...
describe("resolveOutboundTarget channel resolution", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadElysiaClawPlugins.mockReset();
  });
  ...
  it("recovers telegram plugin resolution so announce delivery does not fail with Unsupported channel: telegram", () => {
    ...
    expect(mocks.loadElysiaClawPlugins).toHaveBeenCalledTimes(1);
  });
  ...
  it("retries bootstrap on subsequent resolve when the first bootstrap attempt fails", () => {
    ...
    expect(mocks.loadElysiaClawPlugins).toHaveBeenCalledTimes(2);
  });
});
