...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ElysiaClawConfig | null = null;
  ...
  get(key: string, cfg: ElysiaClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ElysiaClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: ElysiaClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: ElysiaClawConfig): void {
