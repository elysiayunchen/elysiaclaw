...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/elysiaclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/elysiaclaw-state", "devices"),
      pendingPath: path.join("/tmp/elysiaclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/elysiaclaw-state", "devices", "paired.json"),
    });
  });
