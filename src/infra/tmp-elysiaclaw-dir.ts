...
export function resolvePreferredElysiaClawTmpDir(
  options: ResolvePreferredElysiaClawTmpDirOptions = {},
): string {
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback ElysiaClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback ElysiaClaw temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_ELYSIACLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_ELYSIACLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_ELYSIACLAW_TMP_DIR)) {
      return POSIX_ELYSIACLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_ELYSIACLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_ELYSIACLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_ELYSIACLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_ELYSIACLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_ELYSIACLAW_TMP_DIR;
