...
function candidateBinDirs(opts: EnsureElysiaClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ELYSIACLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "elysiaclaw"))) {
...
export function ensureElysiaClawCliOnPath(opts: EnsureElysiaClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ELYSIACLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
