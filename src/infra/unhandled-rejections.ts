...
export function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[elysiaclaw] Unhandled rejection handler failed:",
...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[elysiaclaw] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }

    if (isFatalError(reason)) {
      console.error("[elysiaclaw] FATAL unhandled rejection:", formatUncaughtError(reason));
      process.exit(1);
      return;
    }
    ...
    if (isConfigError(reason)) {
      console.error(
        "[elysiaclaw] CONFIGURATION ERROR - requires fix:",
    ...
    if (isTransientNetworkError(reason)) {
      console.warn(
        "[elysiaclaw] Non-fatal unhandled rejection (continuing):",
        formatUncaughtError(reason),
      );
      return;
    }

    console.error("[elysiaclaw] Unhandled promise rejection:", formatUncaughtError(reason));
    process.exit(1);
  });
}
