...
let ensureElysiaClawCliOnPath: typeof import("./path-env.js").ensureElysiaClawCliOnPath;

describe("ensureElysiaClawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "ELYSIACLAW_PATH_BOOTSTRAPPED",
    "ELYSIACLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureElysiaClawCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/elysiaclaw-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "elysiaclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureElysiaClawCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling elysiaclaw exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.ELYSIACLAW_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.ELYSIACLAW_PATH_BOOTSTRAPPED = "1";
    ensureElysiaClawCliOnPath({
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "elysiaclaw");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.ELYSIACLAW_PATH_BOOTSTRAPPED;
      delete process.env.ELYSIACLAW_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.ELYSIACLAW_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/elysiaclaw-path/case-linuxbrew");
