...
const PRIMARY_PACKAGE_NAME = "elysiaclaw";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "ElysiaClaw", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.ELYSIACLAW_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.ELYSIACLAW_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
