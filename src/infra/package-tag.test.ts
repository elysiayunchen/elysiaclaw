...
describe("normalizePackageTagInput", () => {
  const packageNames = ["elysiaclaw", "@elysiaclaw/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("elysiaclaw@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@elysiaclaw/plugin@2026.2.24", packageNames)).toBe(
      "2026.2.24",
    );
    expect(normalizePackageTagInput("elysiaclaw@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("elysiaclaw", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @elysiaclaw/plugin ", packageNames)).toBeNull();
  });
  ...
  it("returns trimmed raw values when no package prefix matches", () => {
    ...
    expect(normalizePackageTagInput("elysiaclawer@beta", packageNames)).toBe("elysiaclawer@beta");
  });
});
