...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredElysiaClawTmpDir({
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredElysiaClawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredElysiaClawTmpDir", () => {
  it("prefers /tmp/elysiaclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_ELYSIACLAW_TMP_DIR);
  ...
  it("prefers /tmp/elysiaclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ELYSIACLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });
  ...
  it("repairs existing /tmp/elysiaclaw permissions when they are too broad", () => {
    ...
    expect(resolved).toBe(POSIX_ELYSIACLAW_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_ELYSIACLAW_TMP_DIR, 0o700);
  ...
  it("repairs /tmp/elysiaclaw after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_ELYSIACLAW_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_ELYSIACLAW_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ELYSIACLAW_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_ELYSIACLAW_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback ElysiaClaw temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "elysiaclaw");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredElysiaClawTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_ELYSIACLAW_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback ElysiaClaw temp dir/);
  });
});
