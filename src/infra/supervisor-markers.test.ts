  ...
  it("includes the cross-platform supervisor hint env vars", () => {
    expect(SUPERVISOR_HINT_ENV_VARS).toEqual(
      expect.arrayContaining([
        "LAUNCH_JOB_LABEL",
        "INVOCATION_ID",
        "ELYSIACLAW_WINDOWS_TASK_NAME",
        "ELYSIACLAW_SERVICE_MARKER",
        "ELYSIACLAW_SERVICE_KIND",
      ]),
    );
  });
});
...
describe("detectRespawnSupervisor", () => {
  it("detects launchd and systemd only from non-blank platform-specific hints", () => {
    expect(detectRespawnSupervisor({ LAUNCH_JOB_LABEL: " ai.elysiaclaw.gateway " }, "darwin")).toBe(
      "launchd",
    );
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ ELYSIACLAW_WINDOWS_TASK_NAME: "ElysiaClaw Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
    ...
    expect(
      detectRespawnSupervisor(
        {
          ELYSIACLAW_SERVICE_MARKER: "elysiaclaw",
          ELYSIACLAW_SERVICE_KIND: "worker",
        },
        "win32",
      ),
    ).toBeNull();
  ...
  it("ignores service markers on non-Windows platforms and unknown platforms", () => {
    ...
    expect(
      detectRespawnSupervisor({ LAUNCH_JOB_LABEL: "ai.elysiaclaw.gateway" }, "freebsd"),
    ).toBeNull();
  });
});
