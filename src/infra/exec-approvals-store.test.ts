...
afterEach(() => {
  vi.restoreAllMocks();
  if (originalElysiaClawHome === undefined) {
    delete process.env.ELYSIACLAW_HOME;
  } else {
    process.env.ELYSIACLAW_HOME = originalElysiaClawHome;
  }
...
function createHomeDir(): string {
  ...
  tempDirs.push(dir);
  process.env.ELYSIACLAW_HOME = dir;
  return dir;
}

function approvalsFilePath(homeDir: string): string {
  return path.join(homeDir, ".elysiaclaw", "exec-approvals.json");
}
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".elysiaclaw", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".elysiaclaw", "exec-approvals.sock")),
    );
  });
