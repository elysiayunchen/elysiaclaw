...
vi.mock("./elysiaclaw-root.js", () => ({
  resolveElysiaClawPackageRoot: vi.fn(async () => null),
  resolveElysiaClawPackageRootSync: vi.fn(() => null),
}));
...
let elysiaclawRoot: typeof import("./elysiaclaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    elysiaclawRoot = await import("./elysiaclaw-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/elysiaclaw");
    const wrapperArgv1 = abs("fixtures/bin/elysiaclaw");
  ...
  it("uses resolveElysiaClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/elysiaclaw");
    (
      elysiaclawRoot.resolveElysiaClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/elysiaclaw"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "elysiaclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "elysiaclaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-elysiaclaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/elysiaclaw-bundle");
    (
      elysiaclawRoot.resolveElysiaClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/elysiaclaw-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
