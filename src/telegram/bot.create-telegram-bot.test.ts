...
describe("createTelegramBot", () => {
    ...
    for (const testCase of cases) {
      ...
      if (testCase.expectPairingText) {
        ...
        expect(pairingText, testCase.name).toContain(
          "elysiaclaw pairing approve telegram PAIRME12",
        );
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "elysiaclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("honors routed group activation from session store", async () => {
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "elysiaclaw-telegram-"));
