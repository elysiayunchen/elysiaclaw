...
`;
    ...
    expect(parsed.elysiaclaw.emoji).toBe("💾");
    expect(parsed.elysiaclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.elysiaclaw.emoji).toBe("📝");
    expect(parsed.elysiaclaw.events).toEqual(["command"]);
    expect(parsed.elysiaclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.elysiaclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"elysiaclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"elysiaclaw": {"events": ["test"]}}');
  });
...
describe("resolveElysiaClawMetadata", () => {
  it("extracts openclaw metadata from parsed frontmatter", () => {
    ...
    const result = resolveElysiaClawMetadata(frontmatter);
    ...
    "elysiaclaw":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with ElysiaClaw" }],
      },
  }
---
