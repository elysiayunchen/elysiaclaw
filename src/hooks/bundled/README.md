...
This directory contains hooks that ship with ElysiaClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.elysiaclaw/elysiaclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'elysiaclaw.*gateway' && pnpm elysiaclaw gateway`
3. Enable the hook: `elysiaclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.elysiaclaw.ai/automation/hooks
