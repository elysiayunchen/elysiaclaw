...
homepage: https://docs.elysiaclaw.ai/automation/hooks#command-logger
metadata:
  {
    "elysiaclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.elysiaclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.elysiaclaw/logs/commands.log ~/.elysiaclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/elysiaclaw`:
   ```
   /home/username/.elysiaclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
