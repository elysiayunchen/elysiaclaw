...
import type { ElysiaClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: ElysiaClawConfig): string[] {
...
export async function noteSecurityWarnings(cfg: ElysiaClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("elysiaclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.elysiaclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("elysiaclaw approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("elysiaclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("elysiaclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("elysiaclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("elysiaclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('elysiaclaw config set session.dmScope "per-channel-peer"') +
