...
import { writeConfigFile, type ElysiaClawConfig } from "../config/config.js";
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service ELYSIACLAW_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
  ...
  if (!tokenRefConfigured && !configuredGatewayToken && gatewayTokenForRepair) {
    const nextCfg: ElysiaClawConfig = {
      ...cfg,
      gateway: {
        ...cfg.gateway,
        auth: {
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing ElysiaClaw gateway next.");
      }
    }
  }
