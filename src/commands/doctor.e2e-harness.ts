...
export const resolveElysiaClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadElysiaClawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/elysiaclaw.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveElysiaClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ELYSIACLAW_STATE_DIR;
  originalUpdateInProgress = process.env.ELYSIACLAW_UPDATE_IN_PROGRESS;
  process.env.ELYSIACLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "elysiaclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.ELYSIACLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.ELYSIACLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.ELYSIACLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
