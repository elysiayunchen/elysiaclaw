...
function rawKeyValue(config: ElysiaClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: ElysiaClawConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: ElysiaClawConfig, provider: SearchProvider): ElysiaClawConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
export async function setupSearch(
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — ElysiaClaw will store a reference instead of the API key.",
