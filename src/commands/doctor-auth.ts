...
function pruneAuthProfiles(
  cfg: ElysiaClawConfig,
  profileIds: Set<string>,
): { next: ElysiaClawConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("elysiaclaw models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("elysiaclaw models auth setup-token")} or ${formatCliCommand(
  ...
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "elysiaclaw models auth login --provider openai-codex",
    )} or ${formatCliCommand("elysiaclaw configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("elysiaclaw configure")}\` or \`${formatCliCommand("elysiaclaw onboard")}\`.`;
}
