...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  it("uses ELYSIACLAW_GATEWAY_TOKEN when --gateway-token is omitted", async () => {
    await withStateDir("state-env-token-", async (stateDir) => {
      ...
      process.env.ELYSIACLAW_GATEWAY_TOKEN = envToken;
        ...
        await runNonInteractiveOnboarding(
          {
            ...
            gatewayBind: "loopback",
            gatewayAuth: "token",
            gatewayTokenRefEnv: "ELYSIACLAW_GATEWAY_TOKEN",
          },
          runtime,
        );
        ...
        runNonInteractiveOnboarding(
          {
            nonInteractive: true,
            mode: "local",
            workspace: path.join(stateDir, "elysiaclaw"),
            authChoice: "skip",
            skipSkills: true,
            skipHealth: false,
            installDaemon: false,
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `elysiaclaw gateway status --deep` for more detail.");
