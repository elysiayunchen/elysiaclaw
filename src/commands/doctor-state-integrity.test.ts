...
type EnvSnapshot = {
  HOME?: string;
  ELYSIACLAW_HOME?: string;
  ELYSIACLAW_STATE_DIR?: string;
  ELYSIACLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    ELYSIACLAW_HOME: process.env.ELYSIACLAW_HOME,
    ELYSIACLAW_STATE_DIR: process.env.ELYSIACLAW_STATE_DIR,
    ELYSIACLAW_OAUTH_DIR: process.env.ELYSIACLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: ElysiaClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: ElysiaClawConfig) {
...
async function runStateIntegrityText(cfg: ElysiaClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.ELYSIACLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.ELYSIACLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when ELYSIACLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.ELYSIACLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: ElysiaClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
