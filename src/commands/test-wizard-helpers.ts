...
export function requireElysiaClawAgentDir(): string {
  const agentDir = process.env.ELYSIACLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("ELYSIACLAW_AGENT_DIR not set");
  }
  return agentDir;
}
