...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveElysiaClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("elysiaclaw models auth setup-token")}\` or \`${formatCliCommand("elysiaclaw configure")}\`.`
          : `Run \`${formatCliCommand("elysiaclaw configure")}\` or set an API key env var.`;
