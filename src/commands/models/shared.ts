...
export async function loadValidConfigOrThrow(): Promise<ElysiaClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: ElysiaClawConfig) => ElysiaClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ElysiaClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ElysiaClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("elysiaclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
