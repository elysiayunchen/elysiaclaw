...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveElysiaClawAgentDir: vi.fn().mockReturnValue("/tmp/elysiaclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/elysiaclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/elysiaclaw-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/elysiaclaw-dev/elysiaclaw.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveElysiaClawAgentDir: mocks.resolveElysiaClawAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/elysiaclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveElysiaClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/elysiaclaw-dev/elysiaclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/elysiaclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/elysiaclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/elysiaclaw-agent-custom");
