...
function resolveAgentName(cfg: ElysiaClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: ElysiaClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ElysiaClawConfig): AgentSummary[] {
