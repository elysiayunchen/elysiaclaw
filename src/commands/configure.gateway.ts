...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "ELYSIACLAW_GATEWAY_TOKEN",
          placeholder: "ELYSIACLAW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like ELYSIACLAW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. ElysiaClaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: ElysiaClaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.elysiaclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
