...
const ensureElysiaClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveElysiaClawAgentDir = vi.fn().mockReturnValue("/tmp/elysiaclaw-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/elysiaclaw-state",
  loadConfig,
...
vi.mock("../agents/models-config.js", () => ({
  ensureElysiaClawModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveElysiaClawAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureElysiaClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureElysiaClawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureElysiaClawModelsJson).toHaveBeenCalled();
    expect(ensureElysiaClawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
