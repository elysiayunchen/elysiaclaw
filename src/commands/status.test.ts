...
beforeAll(() => {
  envSnapshot = captureEnv(["ELYSIACLAW_PROFILE"]);
  process.env.ELYSIACLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/elysiaclaw/pnpm-lock.yaml",
      markerPath: "/tmp/elysiaclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.elysiaclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.elysiaclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("shows gateway auth when reachable", async () => {
    await withEnvVar("ELYSIACLAW_GATEWAY_TOKEN", "abcd1234", async () => {
