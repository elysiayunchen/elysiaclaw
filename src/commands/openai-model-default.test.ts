...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: ElysiaClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: ElysiaClawConfig) => { changed: boolean; next: ElysiaClawConfig };
  defaultModel: string;
  overrideConfig: ElysiaClawConfig;
  alreadyDefaultConfig: ElysiaClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: ElysiaClawConfig) => config,
      applyDefaultConfig: (config: ElysiaClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: ElysiaClawConfig = { agents: { defaults: {} } };
