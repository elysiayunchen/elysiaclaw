...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.ELYSIACLAW_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("elysiaclaw gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
