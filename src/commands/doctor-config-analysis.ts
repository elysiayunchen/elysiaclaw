...
import { ElysiaClawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: ElysiaClawConfig): {
  config: ElysiaClawConfig;
  removed: string[];
} {
  const parsed = ElysiaClawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: ElysiaClawConfig): void {
