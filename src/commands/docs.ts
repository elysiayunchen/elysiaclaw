...
const SEARCH_TOOL = "https://docs.elysiaclaw.ai/mcp.SearchElysiaClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.elysiaclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('elysiaclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.elysiaclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('elysiaclaw docs "your query"')}`);
    }
    return;
  }
