...
export function normalizeCompatibilityConfigValues(cfg: ElysiaClawConfig): {
  config: ElysiaClawConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: ElysiaClawConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as ElysiaClawConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as ElysiaClawConfig["browser"],
    };
    changes.push(
