...
type GetReplyFromConfigFn = (
  ...
  configOverride?: ElysiaClawConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `elysiaclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.ELYSIACLAW_CONFIG_PATH = path.join(root, "elysiaclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "elysiaclaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "elysiaclaw-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadElysiaClawPlugins: () => pluginRegistryState.registry,
  };
});
