...
export async function withTempConfig(params: {
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.ELYSIACLAW_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.ELYSIACLAW_DISABLE_CONFIG_CACHE;
    } else {
      process.env.ELYSIACLAW_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
