...
const { createElysiaClawTools } = await import("../agents/elysiaclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createElysiaClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createElysiaClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ELYSIACLAW_GATEWAY_PORT", "ELYSIACLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.ELYSIACLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.ELYSIACLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("ELYSIACLAW_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createElysiaClawTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
