...
export function resolveHooksConfig(cfg: ElysiaClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: ElysiaClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-elysiaclaw-token"] === "string"
      ? req.headers["x-elysiaclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-elysiaclaw-idempotency-key"]) ||
