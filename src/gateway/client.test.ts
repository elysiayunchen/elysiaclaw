...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("elysiaclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with ELYSIACLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with ELYSIACLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.ELYSIACLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://elysiaclaw-gateway.ai:18789",
      onConnectError,
    });
