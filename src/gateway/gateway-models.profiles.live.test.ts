...
import { resolveElysiaClawAgentDir } from "../agents/agent-paths.js";
...
import { ensureElysiaClawModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, ElysiaClawConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE =
  isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.ELYSIACLAW_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ELYSIACLAW_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ELYSIACLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ELYSIACLAW_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.ELYSIACLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.ELYSIACLAW_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.ELYSIACLAW_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  cfg: ElysiaClawConfig;
  agentDir: string;
}): ElysiaClawConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<ElysiaClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.ELYSIACLAW_CONFIG_PATH,
    token: process.env.ELYSIACLAW_GATEWAY_TOKEN,
    skipChannels: process.env.ELYSIACLAW_SKIP_CHANNELS,
    skipGmail: process.env.ELYSIACLAW_SKIP_GMAIL_WATCHER,
    skipCron: process.env.ELYSIACLAW_SKIP_CRON,
    skipCanvas: process.env.ELYSIACLAW_SKIP_CANVAS_HOST,
    agentDir: process.env.ELYSIACLAW_AGENT_DIR,
  ...
  process.env.ELYSIACLAW_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.ELYSIACLAW_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveElysiaClawAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-live-state-"));
  process.env.ELYSIACLAW_STATE_DIR = tempStateDir;
  ...
  process.env.ELYSIACLAW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.elysiaclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveElysiaClawAgentDir();
  const sanitizedCfg: ElysiaClawConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-live-"));
  const tempConfigPath = path.join(tempDir, "elysiaclaw.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.ELYSIACLAW_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            const toolProbe = await withGatewayLiveProbeTimeout(
              client.request<AgentFinalPayload>(
                "agent",
                {
                  ...
                  message: strictReply
                    ? "ElysiaClaw live tool probe (local, safe): " +
    ...
    process.env.ELYSIACLAW_CONFIG_PATH = previous.configPath;
    process.env.ELYSIACLAW_GATEWAY_TOKEN = previous.token;
    process.env.ELYSIACLAW_SKIP_CHANNELS = previous.skipChannels;
    process.env.ELYSIACLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ELYSIACLAW_SKIP_CRON = previous.skipCron;
    process.env.ELYSIACLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ELYSIACLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ELYSIACLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureElysiaClawModelsJson(cfg);
      ...
      const rawModels = process.env.ELYSIACLAW_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via ELYSIACLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.elysiaclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
