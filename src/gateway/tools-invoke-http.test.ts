...
let lastCreateElysiaClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/elysiaclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateElysiaClawToolsContext?.agentTo,
          agentThreadId: lastCreateElysiaClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createElysiaClawTools: (ctx: Record<string, unknown>) => {
      lastCreateElysiaClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateElysiaClawToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateElysiaClawToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-elysiaclaw-message-to": "channel:24514",
        "x-elysiaclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
