...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ELYSIACLAW_SKIP_BROWSER_CONTROL_SERVER",
  "ELYSIACLAW_SKIP_GMAIL_WATCHER",
  "ELYSIACLAW_SKIP_CANVAS_HOST",
  "ELYSIACLAW_BUNDLED_PLUGINS_DIR",
  "ELYSIACLAW_SKIP_CHANNELS",
  "ELYSIACLAW_SKIP_PROVIDERS",
  "ELYSIACLAW_SKIP_CRON",
  "ELYSIACLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.ELYSIACLAW_SKIP_PROVIDERS = "1";
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".elysiaclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".elysiaclaw-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["ELYSIACLAW_GATEWAY_TOKEN"]);
  const prev = process.env.ELYSIACLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ELYSIACLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.ELYSIACLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
