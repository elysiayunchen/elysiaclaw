...
const loadElysiaClawPlugins = vi.hoisted(() => vi.fn());
...
function createSubagentRuntime(serverPlugins: ServerPluginsModule): PluginRuntime["subagent"] {
  ...
  loadElysiaClawPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadElysiaClawPlugins.mock.calls.at(-1)?.[0] as
...
beforeEach(() => {
  loadElysiaClawPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadElysiaClawPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("provides subagent runtime with sessions.get method aliases", async () => {
    ...
    const call = loadElysiaClawPlugins.mock.calls.at(-1)?.[0];
