...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/elysiaclaw/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (ElysiaClaw)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.ELYSIACLAW_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "elysiaclaw");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.ELYSIACLAW_TAILNET_DNS?.trim();
