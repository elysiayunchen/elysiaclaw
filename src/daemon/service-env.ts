...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.ELYSIACLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ELYSIACLAW_PROFILE: profile,
    ELYSIACLAW_GATEWAY_PORT: String(port),
    ELYSIACLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    ELYSIACLAW_SYSTEMD_UNIT: systemdUnit,
    ELYSIACLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    ELYSIACLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    ELYSIACLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.ELYSIACLAW_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ELYSIACLAW_GATEWAY_TOKEN: gatewayToken,
    ELYSIACLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    ELYSIACLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    ELYSIACLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    ELYSIACLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    ELYSIACLAW_LOG_PREFIX: "node",
    ELYSIACLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    ELYSIACLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    ELYSIACLAW_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    ELYSIACLAW_STATE_DIR: sharedEnv.stateDir,
    ELYSIACLAW_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.ELYSIACLAW_STATE_DIR;
  const configPath = env.ELYSIACLAW_CONFIG_PATH;
