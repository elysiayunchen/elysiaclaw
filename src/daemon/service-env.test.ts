...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ELYSIACLAW_GATEWAY_PORT).toBe("18789");
    expect(env.ELYSIACLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.ELYSIACLAW_SERVICE_MARKER).toBe("elysiaclaw");
    expect(env.ELYSIACLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ELYSIACLAW_SERVICE_VERSION).toBe("string");
    expect(env.ELYSIACLAW_SYSTEMD_UNIT).toBe("elysiaclaw-gateway.service");
    expect(env.ELYSIACLAW_WINDOWS_TASK_NAME).toBe("ElysiaClaw Gateway");
    if (process.platform === "darwin") {
      expect(env.ELYSIACLAW_LAUNCHD_LABEL).toBe("ai.elysiaclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ELYSIACLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ELYSIACLAW_SYSTEMD_UNIT).toBe("elysiaclaw-gateway-work.service");
    expect(env.ELYSIACLAW_WINDOWS_TASK_NAME).toBe("ElysiaClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.ELYSIACLAW_LAUNCHD_LABEL).toBe("ai.elysiaclaw.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through ELYSIACLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ELYSIACLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.ELYSIACLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to ELYSIACLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.ELYSIACLAW_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers ELYSIACLAW_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".elysiaclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ELYSIACLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".elysiaclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ELYSIACLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".elysiaclaw"));
  });
