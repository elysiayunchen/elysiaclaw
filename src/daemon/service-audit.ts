...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds ELYSIACLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `elysiaclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service ELYSIACLAW_GATEWAY_TOKEN does not match gateway.auth.token in elysiaclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.ELYSIACLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.ELYSIACLAW_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `elysiaclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
