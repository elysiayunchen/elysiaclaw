...
describe("isSystemdServiceEnabled", () => {
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "elysiaclaw-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "elysiaclaw-gateway.service"]);
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled elysiaclaw-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled elysiaclaw-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when ELYSIACLAW_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/elysiaclaw-gateway.service",
    },
    {
      name: "uses profile-specific service name when ELYSIACLAW_PROFILE is set to a custom value",
      env: { HOME: "/home/test", ELYSIACLAW_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/elysiaclaw-gateway-jbphoenix.service",
    },
    {
      name: "prefers ELYSIACLAW_SYSTEMD_UNIT over ELYSIACLAW_PROFILE",
      env: {
        HOME: "/home/test",
        ELYSIACLAW_PROFILE: "jbphoenix",
        ELYSIACLAW_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles ELYSIACLAW_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        ELYSIACLAW_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from ELYSIACLAW_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        ELYSIACLAW_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads ELYSIACLAW_GATEWAY_TOKEN from EnvironmentFile", async () => {
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.ELYSIACLAW_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        ...
        "ExecStart=/usr/bin/elysiaclaw gateway run",
        "EnvironmentFile=%h/.elysiaclaw/.env",
        'Environment="ELYSIACLAW_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.elysiaclaw/.env`]: "ELYSIACLAW_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.ELYSIACLAW_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.ELYSIACLAW_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.elysiaclaw/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/elysiaclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/elysiaclaw gateway run",
          'EnvironmentFile=%h/.elysiaclaw/first.env "%h/.elysiaclaw/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.elysiaclaw/first.env") {
        return "ELYSIACLAW_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.elysiaclaw/second env.env") {
        return 'ELYSIACLAW_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ...
      ELYSIACLAW_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "ELYSIACLAW_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "ELYSIACLAW_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "ELYSIACLAW_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      ELYSIACLAW_GATEWAY_TOKEN: "override-token",
      ELYSIACLAW_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/elysiaclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/elysiaclaw gateway run",
          "EnvironmentFile=%h/.elysiaclaw/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.elysiaclaw/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'ELYSIACLAW_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "ELYSIACLAW_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      ELYSIACLAW_GATEWAY_TOKEN: "quoted token",
      ELYSIACLAW_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      ELYSIACLAW_GATEWAY_TOKEN: "file",
      ELYSIACLAW_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "elysiaclaw-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ ELYSIACLAW_PROFILE: "work" });
  });
