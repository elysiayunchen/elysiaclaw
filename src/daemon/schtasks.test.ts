...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when ELYSIACLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".elysiaclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when ELYSIACLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", ELYSIACLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".elysiaclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers ELYSIACLAW_STATE_DIR over profile-derived defaults",
      env: {
    ...
    },
    {
      ...
      env: { HOME: "/home/test", ELYSIACLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".elysiaclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\ElysiaClaw Share\\node.exe" "\\\\fileserver\\ElysiaClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\ElysiaClaw Share\\node.exe",
            "\\\\fileserver\\ElysiaClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from ELYSIACLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ ELYSIACLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
