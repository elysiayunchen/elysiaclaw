...
function listAgents(cfg: ElysiaClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: ElysiaClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<ElysiaClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: ElysiaClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: ElysiaClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ElysiaClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<ElysiaClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  ElysiaClawConfig,
  {
    bindingsRef: ElysiaClawConfig["bindings"];
    agentsRef: ElysiaClawConfig["agents"];
    sessionRef: ElysiaClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: ElysiaClawConfig): Map<string, ResolvedAgentRoute> {
