...
import { ensureElysiaClawExecMarkerOnProcess } from "./infra/elysiaclaw-exec-env.js";
...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "elysiaclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "elysiaclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.ELYSIACLAW_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.ELYSIACLAW_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.ELYSIACLAW_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.ELYSIACLAW_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[elysiaclaw] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `ElysiaClaw ${VERSION} (${commit})` : `ElysiaClaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[elysiaclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[elysiaclaw] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[elysiaclaw] ${parsed.error}`);
      process.exit(2);
    }
    ...
    if (!tryHandleRootVersionFastPath(process.argv) && !tryHandleRootHelpFastPath(process.argv)) {
      import("./cli/run-main.js")
        ...
        .catch((error) => {
          console.error(
            "[elysiaclaw] Failed to start CLI:",
            error instanceof Error ? (error.stack ?? error.message) : error,
          );
          process.exitCode = 1;
        });
