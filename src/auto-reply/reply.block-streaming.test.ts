...
function createReplyConfig(home: string, streamMode?: "block"): ElysiaClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("elysiaclaw-stream-", async (home) => {
    await fs.mkdir(path.join(home, ".elysiaclaw", "agents", "main", "sessions"), {
      recursive: true,
    });
    return fn(home);
  });
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("ELYSIACLAW_TEST_FAST", "1");
  ...
  it("still parses media directives for direct block payloads", async () => {
    await withTempHome(async (home) => {
      ...
      expect(onBlockReply.mock.calls[0][0]).toMatchObject({
        text: "Result",
        mediaUrls: [path.join(home, "elysiaclaw", "image.png")],
      });
    });
  });
});
