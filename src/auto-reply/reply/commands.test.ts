...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-commands-"));
...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-commands-config-"));
  const configPath = path.join(dir, "elysiaclaw.json");
  const previous = process.env.ELYSIACLAW_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ELYSIACLAW_CONFIG_PATH = previous;
    }
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/elysiaclaw-session-store.json" },
    } as ElysiaClawConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/elysiaclaw-agent-compact";
...
describe("handleCommands /config configWrites gating", () => {
  ...
  it("keeps /config set working for gateway operator.admin clients", async () => {
    await withTempConfigPath({ messages: { ackReaction: ":)" } }, async (configPath) => {
      ...
      const written = await readJsonFile<ElysiaClawConfig>(configPath);
