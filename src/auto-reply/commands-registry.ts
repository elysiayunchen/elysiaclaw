...
export function isCommandEnabled(cfg: ElysiaClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ElysiaClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as ElysiaClawConfig),
...
export function getCommandDetection(_cfg?: ElysiaClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ElysiaClawConfig) {
