...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  ElysiaClaw  ", 20)).toBe("ElysiaClaw");
    expect(coerceIdentityValue("  ElysiaClaw  ", 8)).toBe("ElysiaClaw");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  ElysiaClaw Assistant  ", 8)).toBe("ElysiaClaw");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  ElysiaClaw  ", 0)).toBe("");
    expect(coerceIdentityValue("  ElysiaClaw  ", -1)).toBe("OpenCla");
  });
});
