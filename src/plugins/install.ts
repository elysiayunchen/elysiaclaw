...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing elysiaclaw.extensions; update the plugin package to include elysiaclaw.extensions (for example ["./dist/index.js"]). See https://docs.elysiaclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-elysiaclaw-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_ELYSIACLAW_EXTENSIONS: "missing_elysiaclaw_extensions",
  EMPTY_ELYSIACLAW_EXTENSIONS: "empty_elysiaclaw_extensions",
...
function ensureElysiaClawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_ELYSIACLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json elysiaclaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_ELYSIACLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureElysiaClawExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from elysiaclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-elysiaclaw"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "elysiaclaw security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "elysiaclaw security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "elysiaclaw-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "elysiaclaw-npm-pack-",
    spec,
    timeoutMs,
