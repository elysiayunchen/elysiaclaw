...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-elysiaclaw",
      targetDir: "/tmp/opik-elysiaclaw",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-elysiaclaw": {
              source: "npm",
              spec: "@opik/opik-elysiaclaw",
              integrity: "sha512-old",
              installPath: "/tmp/opik-elysiaclaw",
            },
          },
        },
      },
      pluginIds: ["opik-elysiaclaw"],
      dryRun: true,
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-elysiaclaw": {
              source: "npm",
              spec: "@opik/opik-elysiaclaw@0.2.5",
              integrity: "sha512-old",
              installPath: "/tmp/opik-elysiaclaw",
            },
          },
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @elysiaclaw/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @elysiaclaw/missing.",
      },
    ]);
  });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/elysiaclaw-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",
