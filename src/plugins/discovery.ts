...
import {
  ...
  type ElysiaClawPackageManifest,
...
function resolveDiscoveryCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ELYSIACLAW_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseDiscoveryCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.ELYSIACLAW_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @elysiaclaw/voice-call -> voice-call).
...
export function discoverElysiaClawPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ownershipUid?: number | null;
  cache?: boolean;
