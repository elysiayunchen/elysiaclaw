...
import { discoverElysiaClawPlugins } from "./discovery.js";
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  const modulePath = params.modulePath ?? fileURLToPath(import.meta.url);
  const packageRoot = resolveElysiaClawPackageRootSync({
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`elysiaclaw/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ElysiaClawPluginDefinition;
  register?: ElysiaClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ElysiaClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ElysiaClawPluginDefinition;
...
export function loadElysiaClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverElysiaClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "elysiaclaw/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ElysiaClawPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as ElysiaClawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
