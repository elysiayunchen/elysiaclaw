...
import { clearPluginDiscoveryCache, discoverElysiaClawPlugins } from "./discovery.js";
import {
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverElysiaClawPlugins>[0],
) {
  return discoverElysiaClawPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
describe("discoverElysiaClawPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".elysiaclaw", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".elysiaclaw", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverElysiaClawPlugins({
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverElysiaClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        ELYSIACLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverElysiaClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        ELYSIACLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverElysiaClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        ELYSIACLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
