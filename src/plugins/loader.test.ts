...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.ELYSIACLAW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadElysiaClawPlugins({
      cache: false,
  ...
  process.env.ELYSIACLAW_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.ELYSIACLAW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadElysiaClawPlugins>) {
...
function useNoBundledPlugins() {
  process.env.ELYSIACLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadElysiaClawPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.ELYSIACLAW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.ELYSIACLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadElysiaClawPlugins", () => {
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadElysiaClawPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadElysiaClawPlugins(options);
  ...
  it("does not reuse cached bundled plugin registries across env changes", () => {
    ...
    const first = loadElysiaClawPlugins({
      ...options,
      env: {
        ...process.env,
        ELYSIACLAW_BUNDLED_PLUGINS_DIR: bundledA,
      },
    });
    const second = loadElysiaClawPlugins({
      ...options,
      env: {
        ...process.env,
        ELYSIACLAW_BUNDLED_PLUGINS_DIR: bundledB,
      },
    });
  ...
  it("does not reuse cached registries when env-resolved install paths change", () => {
    ...
    const first = loadElysiaClawPlugins({
      ...
      env: {
        ...
        ELYSIACLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
      },
    });
    ...
    const second = loadElysiaClawPlugins(secondOptions);
    const third = loadElysiaClawPlugins(secondOptions);
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadElysiaClawPlugins({
      env: {
        ...
        HOME: homeDir,
        ELYSIACLAW_HOME: undefined,
        ELYSIACLAW_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    const stateDir = makeTempDir();
    withEnv({ ELYSIACLAW_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  it("lets an explicitly trusted workspace plugin shadow a bundled plugin with the same id", () => {
    ...
    const workspaceDir = makeTempDir();
    const workspaceExtDir = path.join(workspaceDir, ".elysiaclaw", "extensions", "shadowed");
    mkdirSafe(workspaceExtDir);
    writePlugin({
      id: "shadowed",
      body: `module.exports = { id: "shadowed", register() {} };`,
  ...
  register() {},
    ...
    const script = `
      import { loadElysiaClawPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadElysiaClawPlugins({
        cache: false,
