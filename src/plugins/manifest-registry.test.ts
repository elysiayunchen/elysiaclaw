...
function makeTempDir() {
  return makeTrackedTempDir("elysiaclaw-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "elysiaclaw.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "elysiaclaw.plugin.json");
  const linkedManifest = path.join(rootDir, "elysiaclaw.plugin.json");
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: {
        ...
        ELYSIACLAW_STATE_DIR: path.join(homeA, ".state"),
      },
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: {
        ...
        HOME: homeB,
        ELYSIACLAW_HOME: undefined,
        ELYSIACLAW_STATE_DIR: path.join(homeB, ".state"),
      },
    });
