...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "elysiaclaw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".elysiaclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<ElysiaClawConfig> = {},
): ElysiaClawConfig {
  const base: ElysiaClawConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<ElysiaClawConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<ElysiaClawConfig>,
