...
const { makeStorePath } = createCronStoreHarness({ prefix: "elysiaclaw-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ELYSIACLAW_HOME for tilde expansion", () => {
    vi.stubEnv("ELYSIACLAW_HOME", "/srv/elysiaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/elysiaclaw-home", "cron", "jobs.json"));
  });
});
