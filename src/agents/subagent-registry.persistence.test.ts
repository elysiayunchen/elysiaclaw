...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["ELYSIACLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-subagent-"));
  ...
  it("uses isolated temp state when ELYSIACLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "elysiaclaw-test-state"));
  });
});
