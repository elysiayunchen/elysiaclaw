...
export async function withTempWorkspace(
  ...
  const tempHome = await createTempHomeEnv("elysiaclaw-skills-install-home-");
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-skills-install-"));
  try {
