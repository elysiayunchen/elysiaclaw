...
function resolveSkillsLimits(config?: ElysiaClawConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "elysiaclaw-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "elysiaclaw-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveElysiaClawMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: ElysiaClawConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`elysiaclaw skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
