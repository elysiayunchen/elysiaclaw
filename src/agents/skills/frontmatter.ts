...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseElysiaClawManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyElysiaClawManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveElysiaClawMetadata(
  frontmatter: ParsedSkillFrontmatter,
): ElysiaClawSkillMetadata | undefined {
  const metadataObj = resolveElysiaClawManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveElysiaClawManifestRequires(metadataObj);
  const install = resolveElysiaClawManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveElysiaClawManifestOs(metadataObj);
  return {
