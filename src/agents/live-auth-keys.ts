...
const GOOGLE_LIVE_SINGLE_KEY = "ELYSIACLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ELYSIACLAW_LIVE_ANTHROPIC_KEY",
    listVar: "ELYSIACLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ELYSIACLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ELYSIACLAW_LIVE_${base}_KEY`;
