...
const originalBundledDir = process.env.ELYSIACLAW_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("elysiaclaw-bundled-");
  const workspaceDir = await createTempDir("elysiaclaw-workspace-");
...
afterEach(async () => {
  process.env.ELYSIACLAW_BUNDLED_PLUGINS_DIR = originalBundledDir;
...
describe("resolveEmbeddedRunSkillEntries (integration)", () => {
  it("loads bundled diffs skill when explicitly enabled in config", async () => {
    ...
    process.env.ELYSIACLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
    clearPluginManifestRegistryCache();
