...
function findTool(name: string, config: ElysiaClawConfig) {
  const allTools = createElysiaClawTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: ElysiaClawConfig; env?: NodeJS.ProcessEnv }) {
