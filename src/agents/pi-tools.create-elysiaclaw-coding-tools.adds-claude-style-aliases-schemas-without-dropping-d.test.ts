...
describe("createElysiaClawCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-read-"));
    try {
      ...
      const contents = "Hello from elysiaclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createElysiaClawCodingTools({ sandbox });
