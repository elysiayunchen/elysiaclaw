...
import { markElysiaClawExecEnv } from "../../infra/elysiaclaw-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "elysiaclaw.sandbox=1");
  args.push("--label", `elysiaclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `elysiaclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `elysiaclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markElysiaClawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "elysiaclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`elysiaclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`elysiaclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("elysiaclaw sandbox recreate --all");
}
