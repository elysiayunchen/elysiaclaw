...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    elysiaclaw: process.env.ELYSIACLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.elysiaclaw === undefined) {
      delete process.env.ELYSIACLAW_GATEWAY_TOKEN;
    } else {
      process.env.ELYSIACLAW_GATEWAY_TOKEN = envSnapshot.elysiaclaw;
    }
  ...
  it("uses ELYSIACLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.ELYSIACLAW_GATEWAY_TOKEN = "local-env-token";
