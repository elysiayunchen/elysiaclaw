...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): ElysiaClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as ElysiaClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): ElysiaClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as ElysiaClawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: ElysiaClawConfig, accountId = "media") {
    await handleTelegramAction(
