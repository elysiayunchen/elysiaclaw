...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ELYSIACLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".elysiaclaw", `workspace-${profile}`);
  }
  return path.join(home, ".elysiaclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".elysiaclaw";
