...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: ElysiaClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: ElysiaClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: ElysiaClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: ElysiaClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: ElysiaClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: ElysiaClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: ElysiaClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("uses ELYSIACLAW_HOME for default agent workspace", () => {
    ...
    expect(workspace).toBe(path.join(path.resolve(home), ".elysiaclaw", "workspace"));
  });
  ...
  it("uses ELYSIACLAW_HOME for default agentDir", () => {
    ...
    const agentDir = resolveAgentDir({} as ElysiaClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".elysiaclaw", "agents", "main", "agent"));
  });
});
