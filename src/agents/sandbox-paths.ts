...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const elysiaclawTmpDir = path.resolve(resolvePreferredElysiaClawTmpDir());
  if (!isPathInside(elysiaclawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: elysiaclawTmpDir });
  return resolved;
}
