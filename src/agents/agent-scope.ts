...
type AgentEntry = NonNullable<NonNullable<ElysiaClawConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: ElysiaClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: ElysiaClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: ElysiaClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: ElysiaClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: ElysiaClawConfig, agentId: string): string | undefined {
...
export function resolveRunModelFallbacksOverride(params: {
  cfg: ElysiaClawConfig | undefined;
  agentId?: string | null;
  sessionKey?: string | null;
}): string[] | undefined {
  if (!params.cfg) {
...
export function resolveAgentWorkspaceDir(cfg: ElysiaClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: ElysiaClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
