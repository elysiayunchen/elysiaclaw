...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("elysiaclaw-ws-", async (workspaceDir) => {
      await withTempDir("elysiaclaw-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createElysiaClawCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("elysiaclaw-ws-", async (workspaceDir) => {
      await withTempDir("elysiaclaw-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("elysiaclaw-ws-", async (workspaceDir) => {
      const cfg: ElysiaClawConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createElysiaClawCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-elysiaclaw.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("elysiaclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("elysiaclaw-workspace-", async (workspaceDir) => {
        ...
        const tools = createElysiaClawCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
