...
import { planElysiaClawModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: ElysiaClawConfig): {
  config: ElysiaClawConfig;
  sourceConfigForSecrets: ElysiaClawConfig;
} {
...
export async function ensureElysiaClawModelsJson(
  config?: ElysiaClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveElysiaClawAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planElysiaClawModelsJson({
      cfg,
