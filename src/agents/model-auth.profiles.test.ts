...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-oauth-"));

    try {
      const agentDir = path.join(tempDir, "agent");
      await withEnvAsync(
        {
          ELYSIACLAW_STATE_DIR: tempDir,
          ELYSIACLAW_AGENT_DIR: agentDir,
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.ELYSIACLAW_AGENT_DIR, {
