...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ELYSIACLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ELYSIACLAW_HOME: "/srv/elysiaclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/elysiaclaw-home"), ".elysiaclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".elysiaclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("elysiaclaw-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-workspace-hardlink-"));
    try {
