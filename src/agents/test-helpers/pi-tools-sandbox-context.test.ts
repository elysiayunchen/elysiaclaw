...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("elysiaclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("elysiaclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("elysiaclaw-sbx-");
