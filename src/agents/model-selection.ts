...
export function isCliProvider(provider: string, cfg?: ElysiaClawConfig): boolean {
  const normalized = normalizeProviderId(provider);
...
function resolveAllowedFallbacks(params: { cfg: ElysiaClawConfig; agentId?: string }): string[] {
  if (params.agentId) {
