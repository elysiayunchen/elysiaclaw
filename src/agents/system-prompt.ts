...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; ElysiaClaw handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "When diagnosing issues, run `elysiaclaw status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running ElysiaClaw process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List ElysiaClaw agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List ElysiaClaw agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside ElysiaClaw.";
  }

  const lines = [
    "You are a personal assistant running inside ElysiaClaw.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control ElysiaClaw's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    // ═══════════════════════════════════════════════════════════════
    // CORE: Claude Code-style Behavior Guidance
    // ═══════════════════════════════════════════════════════════════
    "## Doing Tasks",
    ...
    "",
    // ═══════════════════════════════════════════════════════════════
    // BEHAVIOR: Plan Mode, Hooks, System Reminders, Communication
    // ═══════════════════════════════════════════════════════════════
    "## Plan Mode",
    "For complex tasks, use enter_plan_mode PROACTIVELY when:",
    ...
    "",
    "Do NOT use plan mode for simple tasks: typos, obvious bugs, small tweaks, single-function additions, or tasks with very specific instructions.",
    "",
    "In plan mode: read/analyze files → create a plan with todo_write → present to user → wait for approval → call exit_plan_mode.",
    ...
    "If unsure, ask the user to run `elysiaclaw help` (or `elysiaclaw gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## ElysiaClaw Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, ElysiaClaw pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by ElysiaClaw and included below in Project Context.",
    "",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'ElysiaClaw treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
