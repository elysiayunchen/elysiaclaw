...
declare const __ELYSIACLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "elysiaclaw";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["ELYSIACLAW_VERSION"],
      runtimeVersion,
      env["ELYSIACLAW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current ElysiaClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __ELYSIACLAW_VERSION__ === "string" ? __ELYSIACLAW_VERSION__ : undefined,
  bundledVersion: process.env.ELYSIACLAW_BUNDLED_VERSION,
});
