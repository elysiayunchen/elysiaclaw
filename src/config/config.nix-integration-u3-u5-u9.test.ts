...
function loadConfigForHome(home: string) {
  return createConfigIO({
    env: envWith({ ELYSIACLAW_HOME: home }),
    homedir: () => home,
  }).loadConfig();
}
...
describe("Nix integration (U3, U5, U9)", () => {
  describe("U3: isNixMode env var detection", () => {
    it("isNixMode is false when ELYSIACLAW_NIX_MODE is not set", () => {
      expect(resolveIsNixMode(envWith({ ELYSIACLAW_NIX_MODE: undefined }))).toBe(false);
    });

    it("isNixMode is false when ELYSIACLAW_NIX_MODE is empty", () => {
      expect(resolveIsNixMode(envWith({ ELYSIACLAW_NIX_MODE: "" }))).toBe(false);
    });

    it("isNixMode is false when ELYSIACLAW_NIX_MODE is not '1'", () => {
      expect(resolveIsNixMode(envWith({ ELYSIACLAW_NIX_MODE: "true" }))).toBe(false);
    });

    it("isNixMode is true when ELYSIACLAW_NIX_MODE=1", () => {
      expect(resolveIsNixMode(envWith({ ELYSIACLAW_NIX_MODE: "1" }))).toBe(true);
    });
  });
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    ...
    it("STATE_DIR respects ELYSIACLAW_STATE_DIR override", () => {
      expect(resolveStateDir(envWith({ ELYSIACLAW_STATE_DIR: "/custom/state/dir" }))).toBe(
        path.resolve("/custom/state/dir"),
      );
    });

    it("STATE_DIR respects ELYSIACLAW_HOME when state override is unset", () => {
      const customHome = path.join(path.sep, "custom", "home");
      expect(
        resolveStateDir(envWith({ ELYSIACLAW_HOME: customHome, ELYSIACLAW_STATE_DIR: undefined })),
    ...
    it("CONFIG_PATH defaults to ELYSIACLAW_HOME/.elysiaclaw/elysiaclaw.json", () => {
      ...
      expect(
        resolveConfigPathCandidate(
          envWith({
            ELYSIACLAW_HOME: customHome,
            ELYSIACLAW_CONFIG_PATH: undefined,
    ...
    it("CONFIG_PATH expands ~ in ELYSIACLAW_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        expect(
          resolveConfigPathCandidate(
            envWith({ ELYSIACLAW_HOME: home, ELYSIACLAW_CONFIG_PATH: "~/.elysiaclaw/custom.json" }),
            () => home,
          ),
        ).toBe(path.join(home, ".elysiaclaw", "custom.json"));
      });
    });
    ...
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
          ...
          JSON.stringify(
            {
              ...
              channels: {
                whatsapp: {
                  accounts: {
                    personal: {
                      authDir: "~/.elysiaclaw/credentials/wa-personal",
                    },
                  },
                },
              },
        ...
        expect(cfg.agents?.list?.[0]?.agentDir).toBe(
          path.join(home, ".elysiaclaw", "agents", "main"),
        );
        expect(cfg.agents?.list?.[0]?.sandbox?.workspaceRoot).toBe(path.join(home, "sandbox-root"));
        expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
          path.join(home, ".elysiaclaw", "credentials", "wa-personal"),
        );
      });
    });
  });
  ...
  describe("U6: gateway port resolution", () => {
    it("uses default when env and config are unset", () => {
      expect(resolveGatewayPort({}, envWith({ ELYSIACLAW_GATEWAY_PORT: undefined }))).toBe(
        DEFAULT_GATEWAY_PORT,
      );
    });

    it("prefers ELYSIACLAW_GATEWAY_PORT over config", () => {
      expect(
        resolveGatewayPort(
          { gateway: { port: 19002 } },
          envWith({ ELYSIACLAW_GATEWAY_PORT: "19001" }),
        ),
      ).toBe(19001);
    });
    ...
    it("falls back to config when env is invalid", () => {
      expect(
        resolveGatewayPort(
          { gateway: { port: 19003 } },
          envWith({ ELYSIACLAW_GATEWAY_PORT: "nope" }),
        ),
      ).toBe(19003);
    });
  });
