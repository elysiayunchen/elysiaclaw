...
import type { ConfigFileSnapshot } from "./types.elysiaclaw.js";
import { ElysiaClawSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(ElysiaClawSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.elysiaclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/elysiaclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/elysiaclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/elysiaclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/elysiaclaw/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = ElysiaClawSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "ElysiaClawConfig";
    const hints = mainSchemaHints;
