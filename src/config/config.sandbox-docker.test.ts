...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("elysiaclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "elysiaclaw-sandbox-browser-global" },
      agentBrowser: { network: "elysiaclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("elysiaclaw-sandbox-browser-agent");
  });
