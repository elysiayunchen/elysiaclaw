...
import type { ElysiaClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ElysiaClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: ElysiaClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: ElysiaClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ElysiaClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ElysiaClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ElysiaClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as ElysiaClawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as ElysiaClawConfig,
  };
}
