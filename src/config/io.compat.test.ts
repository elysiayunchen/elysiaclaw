...
describe("config io paths", () => {
  ...
  it("uses ELYSIACLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
  ...
  it("honors explicit ELYSIACLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".elysiaclaw", 20002, "custom.json");
      const io = createIoForHome(home, { ELYSIACLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("honors legacy CLAWDBOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".elysiaclaw", 20003, "legacy-custom.json");
