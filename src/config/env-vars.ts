...
function collectConfigEnvVarsByTarget(cfg?: ElysiaClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: ElysiaClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: ElysiaClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: ElysiaClawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.ELYSIACLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
