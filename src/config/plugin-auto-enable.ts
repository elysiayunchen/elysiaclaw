...
function isWhatsAppConfigured(cfg: ElysiaClawConfig): boolean {
...
function isGenericChannelConfigured(cfg: ElysiaClawConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: ElysiaClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: ElysiaClawConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: ElysiaClawConfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: ElysiaClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: ElysiaClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: ElysiaClawConfig, pluginId: string): ElysiaClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
