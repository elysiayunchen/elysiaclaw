...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "elysiaclaw.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      ELYSIACLAW_STATE_DIR: path.join(suiteHome, ".elysiaclaw"),
      CLAWDBOT_STATE_DIR: undefined,
      ELYSIACLAW_PLUGIN_MANIFEST_CACHE_MS: "10000",
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-config-plugin-validation-"));
