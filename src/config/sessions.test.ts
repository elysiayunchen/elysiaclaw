...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ELYSIACLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-elysiaclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-elysiaclaw#general");
  });
  ...
  it("derives session transcripts dir from ELYSIACLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ELYSIACLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when ELYSIACLAW_STATE_DIR differs from stored paths", () => {
