...
import type { ElysiaClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareElysiaClawVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  elysiaclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  elysiaclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: ElysiaClawConfig,
  pathSegments: string[],
): { changed: boolean; next: ElysiaClawConfig } {
...
function coerceConfig(value: unknown): ElysiaClawConfig {
...
function stampConfigVersion(cfg: ElysiaClawConfig): ElysiaClawConfig {
...
function warnIfConfigFromFuture(cfg: ElysiaClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareElysiaClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer ElysiaClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as ElysiaClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ElysiaClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ElysiaClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ElysiaClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as ElysiaClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ELYSIACLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.ELYSIACLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.ELYSIACLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.ELYSIACLAW_WATCH_SESSION === "string" &&
        deps.env.ELYSIACLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.ELYSIACLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.ELYSIACLAW_WATCH_COMMAND === "string" &&
        deps.env.ELYSIACLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.ELYSIACLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `ELYSIACLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: ElysiaClawConfig | null = null;
let runtimeConfigSourceSnapshot: ElysiaClawConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ELYSIACLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ELYSIACLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): ElysiaClawConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): ElysiaClawConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: ElysiaClawConfig;
  candidate: ElysiaClawConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: ElysiaClawConfig): ElysiaClawConfig {
...
export function loadConfig(): ElysiaClawConfig {
...
export async function readBestEffortConfig(): Promise<ElysiaClawConfig> {
...
export async function writeConfigFile(
  ...
  // edits to elysiaclaw.json remain visible (no stale snapshot).
}
