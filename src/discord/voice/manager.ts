...
function resolveVoiceTtsConfig(params: { cfg: ElysiaClawConfig; override?: TtsConfig }): {
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredElysiaClawTmpDir(), "discord-voice-"));
