...
describe("agent components", () => {
  const createCfg = (): ElysiaClawConfig => ({}) as ElysiaClawConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): ElysiaClawConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/elysiaclaw-sessions-test.json");
  });
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    ...
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/elysiaclaw",
    });
  });
});
