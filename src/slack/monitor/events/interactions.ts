...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${ELYSIACLAW_ACTION_PREFIX}`);

  // Handle ElysiaClaw modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
