...
val androidStoreFile = providers.gradleProperty("ELYSIACLAW_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("ELYSIACLAW_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("ELYSIACLAW_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("ELYSIACLAW_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set ELYSIACLAW_ANDROID_STORE_FILE, " +
            "ELYSIACLAW_ANDROID_STORE_PASSWORD, ELYSIACLAW_ANDROID_KEY_ALIAS, and " +
            "ELYSIACLAW_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/ElysiaClawKit/Sources/ElysiaClawKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.elysiaclaw.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "elysiaclaw-$versionName-$buildType.apk"
