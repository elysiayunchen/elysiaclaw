...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    private func voiceWakeChangedEvent(payload: ElysiaClawProtocol.AnyCodable) -> EventFrame {
        EventFrame(
            type: "event",
    ...
    @Test func `applies voice wake changed event to app state`() async {
        ...
        let evt = self.voiceWakeChangedEvent(payload: ElysiaClawProtocol.AnyCodable(["triggers": [
            "elysiaclaw",
            "computer",
        ]]))
        ...
        #expect(updated == ["elysiaclaw", "computer"])
    ...
    @Test func `ignores voice wake changed event with invalid payload`() async {
        let previous = await applyTriggersAndCapturePrevious(["before"])
        let evt = self.voiceWakeChangedEvent(payload: ElysiaClawProtocol.AnyCodable(["unexpected": 123]))
