...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers open claw binary`() throws {
        ...
        let elysiaclawPath = tmp.appendingPathComponent("node_modules/.bin/elysiaclaw")
        try makeExecutableForTests(at: elysiaclawPath)

        let cmd = CommandResolver.elysiaclawCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([elysiaclawPath.path, "gateway"]))
    }
    ...
    @Test func `prefers open claw binary over pnpm`() throws {
        ...
        let elysiaclawPath = binDir.appendingPathComponent("elysiaclaw")
        ...
        #expect(cmd.prefix(2).elementsEqual([elysiaclawPath.path, "rpc"]))
    }
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "elysiaclaw", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "elysiaclaw", "health", "--json"]))
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("elysiaclaw@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/elysiaclaw", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "elysiaclaw@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/elysiaclaw'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("elysiaclaw"))
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == elysiaclawPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
