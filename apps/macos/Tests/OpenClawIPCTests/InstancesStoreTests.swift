...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: ElysiaClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ElysiaClawProtocol.AnyCodable] = [
            "presence": .init([ElysiaClawProtocol.AnyCodable(entry)]),
        ]
        let payload = ElysiaClawProtocol.AnyCodable(payloadMap)
