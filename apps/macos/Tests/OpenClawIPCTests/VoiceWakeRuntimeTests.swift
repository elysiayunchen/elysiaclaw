...
struct VoiceWakeRuntimeTests {
    @Test func `trims after trigger keeps post speech`() {
        let triggers = ["claude", "elysiaclaw"]
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["elysiaclaw"]
        let text = "hey elysiaclaw"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey elysiaclaw do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["elysiaclaw"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "elysiaclaw", start: 0.2, duration: 0.1, range: transcript.range(of: "elysiaclaw")),
