...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.elysiaclaw.mac`) even if the shipped
    ...
    {
        if environment["ELYSIACLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "elysiaclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "elysiaclaw.nixMode") { return true }
