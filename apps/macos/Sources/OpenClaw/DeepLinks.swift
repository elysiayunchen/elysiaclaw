...
private let deepLinkLogger = Logger(subsystem: "ai.elysiaclaw", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "ElysiaClaw is paused", message: "Unpause ElysiaClaw to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run ElysiaClaw agent?", message: body) else { return }
        }
