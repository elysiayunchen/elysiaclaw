...
enum DebugActions {
    private static let verboseDefaultsKey = "elysiaclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "elysiaclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ElysiaClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "ElysiaClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ElysiaClawPaths.configURL
        guard
