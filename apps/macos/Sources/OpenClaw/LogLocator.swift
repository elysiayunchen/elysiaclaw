...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ELYSIACLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/elysiaclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("elysiaclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("elysiaclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/elysiaclaw/ (rolling or stdout), or nil if none exist.
