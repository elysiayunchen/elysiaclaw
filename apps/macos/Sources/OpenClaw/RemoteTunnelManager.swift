...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.elysiaclaw", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous ElysiaClaw run already has an SSH listener on the expected port (common after restarts),
