    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("Allow these so ElysiaClaw can notify and capture when needed.")
                    .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = ElysiaClawLocationMode.off.rawValue
    ...
    private var locationMode: ElysiaClawLocationMode {
        ElysiaClawLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: ElysiaClawLocationMode) async -> Bool {
