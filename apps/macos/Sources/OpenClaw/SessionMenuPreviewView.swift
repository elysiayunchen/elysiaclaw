...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.elysiaclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ElysiaClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ElysiaClawSessionPreviewEntry,
    ...
    private static func cache(payload: ElysiaClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ElysiaClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ElysiaClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ElysiaClawKit.AnyCodable]) -> [ElysiaClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ElysiaClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ElysiaClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ElysiaClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ElysiaClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ElysiaClawChatMessage) -> String? {
