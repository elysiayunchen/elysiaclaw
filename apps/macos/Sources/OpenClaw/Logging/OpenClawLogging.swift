...
enum ElysiaClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = ElysiaClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ElysiaClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ElysiaClawLogging.bootstrapIfNeeded()
        let label = ElysiaClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ElysiaClawOSLogHandler: AppLogLevelBackedHandler {
...
struct ElysiaClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ElysiaClawLogging.parseLabel(self.label)
