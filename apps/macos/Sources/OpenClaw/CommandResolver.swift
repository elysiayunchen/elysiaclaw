...
enum CommandResolver {
    private static let projectRootDefaultsKey = "elysiaclaw.gatewayProjectRootPath"
    private static let helperName = "elysiaclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let elysiaclawEntry = root.appendingPathComponent("elysiaclaw.mjs").path
        if FileManager().isReadableFile(atPath: elysiaclawEntry) { return elysiaclawEntry }
        let binEntry = root.appendingPathComponent("bin/elysiaclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/elysiaclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let elysiaclawPaths = self.elysiaclawManagedPaths(home: home)
        if !elysiaclawPaths.isEmpty {
            extras.insert(contentsOf: elysiaclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + elysiaclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func elysiaclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".elysiaclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func elysiaclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectElysiaClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("elysiaclaw.mjs").path,
            root.appendingPathComponent("bin/elysiaclaw.js").path,
        ]
    ...
    static func hasAnyElysiaClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.elysiaclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func elysiaclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let elysiaclawPath = self.projectElysiaClawExecutable(projectRoot: root) {
            return [elysiaclawPath, subcommand] + extraArgs
        }
        if let elysiaclawPath = self.elysiaclawExecutable(searchPaths: searchPaths) {
            return [elysiaclawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "elysiaclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            elysiaclaw entrypoint missing (looked for dist/index.js or elysiaclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real elysiaclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/elysiaclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v elysiaclaw >/dev/null 2>&1; then
          CLI="$(command -v elysiaclaw)"
          elysiaclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/elysiaclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/elysiaclaw.mjs"
            node "$PRJ/elysiaclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/elysiaclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/elysiaclaw.js"
            node "$PRJ/bin/elysiaclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent elysiaclaw"
          pnpm --silent elysiaclaw \(quotedArgs);
        else
          echo "elysiaclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ElysiaClawConfigFile.loadDict()
