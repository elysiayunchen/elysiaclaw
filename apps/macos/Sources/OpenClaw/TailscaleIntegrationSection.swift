...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.elysiaclaw/elysiaclaw.json. Prefer ELYSIACLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.elysiaclaw/elysiaclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.elysiaclaw/elysiaclaw.json. Restart the gateway to apply."
        }
