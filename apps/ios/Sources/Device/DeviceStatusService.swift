...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> ElysiaClawDeviceStatusPayload {
        ...
        return ElysiaClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> ElysiaClawDeviceInfoPayload {
        ...
        return ElysiaClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> ElysiaClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: ElysiaClawBatteryState = switch device.batteryState {
        ...
        return ElysiaClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> ElysiaClawThermalStatusPayload {
        let state: ElysiaClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return ElysiaClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> ElysiaClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return ElysiaClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
