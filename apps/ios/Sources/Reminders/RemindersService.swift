...
final class RemindersService: RemindersServicing {
    func list(params: ElysiaClawRemindersListParams) async throws -> ElysiaClawRemindersListPayload {
        ...
        let payload: [ElysiaClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return ElysiaClawReminderPayload(
        ...
        return ElysiaClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ElysiaClawRemindersAddParams) async throws -> ElysiaClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ElysiaClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ElysiaClawRemindersAddPayload(reminder: payload)
    }
