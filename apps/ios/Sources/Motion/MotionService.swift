...
final class MotionService: MotionServicing {
    func activities(params: ElysiaClawMotionActivityParams) async throws -> ElysiaClawMotionActivityPayload {
        ...
        let mapped: [ElysiaClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return ElysiaClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: ElysiaClawPedometerParams) async throws -> ElysiaClawPedometerPayload {
        ...
        let payload: ElysiaClawPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = ElysiaClawPedometerPayload(
