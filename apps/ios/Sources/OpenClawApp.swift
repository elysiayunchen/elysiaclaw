...
@MainActor
final class ElysiaClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.elysiaclaw.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.elysiaclaw.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.elysiaclaw.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "elysiaclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "elysiaclaw.watch.promptId"
    static let sessionKeyKey = "elysiaclaw.watch.sessionKey"
    static let actionPrimaryIDKey = "elysiaclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "elysiaclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "elysiaclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "elysiaclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "elysiaclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "elysiaclaw.watch.action.secondary"
    static let actionIdentifierPrefix = "elysiaclaw.watch.action."
    static let actionIDKeyPrefix = "elysiaclaw.watch.action.id."
    static let actionLabelKeyPrefix = "elysiaclaw.watch.action.label."
    static let categoryPrefix = "elysiaclaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: ElysiaClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> ElysiaClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return ElysiaClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "ElysiaClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [ElysiaClawWatchAction]) -> [UNNotificationAction] {
...
@main
struct ElysiaClawApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(ElysiaClawAppDelegate.self) private var appDelegate
...
extension ElysiaClawApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("ElysiaClaw: installing uncaught exception handler")
