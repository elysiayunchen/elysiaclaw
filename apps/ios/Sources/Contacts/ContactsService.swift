...
final class ContactsService: ContactsServicing {
    ...
    func search(params: ElysiaClawContactsSearchParams) async throws -> ElysiaClawContactsSearchPayload {
        ...
        return ElysiaClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: ElysiaClawContactsAddParams) async throws -> ElysiaClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return ElysiaClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return ElysiaClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> ElysiaClawContactPayload {
        ElysiaClawContactPayload(
