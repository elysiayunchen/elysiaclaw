...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__ELYSIACLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<ElysiaClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.elysiaclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = ElysiaClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> ElysiaClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return ElysiaClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> ElysiaClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> ElysiaClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return ElysiaClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> ElysiaClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = ElysiaClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: ElysiaClawTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: ElysiaClawAgentEventPayload.self
            ) else {
                continue
            }
