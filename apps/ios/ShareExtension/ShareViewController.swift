...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.elysiaclaw.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to ElysiaClaw", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to ElysiaClaw gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to ElysiaClaw.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "ElysiaClawShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "ElysiaClaw is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "ElysiaClaw Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("elysiaclaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
