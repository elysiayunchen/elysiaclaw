...
@MainActor
public struct ElysiaClawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: ElysiaClawChatViewModel
    ...
    public init(
        viewModel: ElysiaClawChatViewModel,
    ...
    public var body: some View {
        ZStack {
            if self.style == .standard {
                ElysiaClawChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    private var visibleMessages: [ElysiaClawChatMessage] {
        let base: [ElysiaClawChatMessage]
    ...
    private func mergeToolResults(in messages: [ElysiaClawChatMessage]) -> [ElysiaClawChatMessage] {
        ...
        for message in messages {
            ...
            let merged = ElysiaClawChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: ElysiaClawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: ElysiaClawChatMessage) -> Bool {
    ...
    private func primaryText(in message: ElysiaClawChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: ElysiaClawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: ElysiaClawChatMessage) -> [ElysiaClawChatMessageContent] {
    ...
    private func inlineToolResults(in message: ElysiaClawChatMessage) -> [ElysiaClawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: ElysiaClawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: ElysiaClawChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(ElysiaClawChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
