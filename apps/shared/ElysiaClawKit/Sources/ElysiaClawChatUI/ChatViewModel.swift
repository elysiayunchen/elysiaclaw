...
private let chatUILogger = Logger(subsystem: "ai.elysiaclaw", category: "ElysiaClawChatUI")

@MainActor
@Observable
public final class ElysiaClawChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [ElysiaClawChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [ElysiaClawChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [ElysiaClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ElysiaClawChatPendingToolCall] = []
    public private(set) var sessions: [ElysiaClawChatSessionEntry] = []
    private let transport: any ElysiaClawChatTransport
    private var sessionDefaults: ElysiaClawChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: ElysiaClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any ElysiaClawChatTransport,
    ...
    public var sessionChoices: [ElysiaClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [ElysiaClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ElysiaClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ElysiaClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ElysiaClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: ElysiaClawChatMessage) -> ElysiaClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> ElysiaClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return ElysiaClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: ElysiaClawChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [ElysiaClawChatMessage],
        incoming: [ElysiaClawChatMessage]) -> [ElysiaClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [ElysiaClawChatMessage]) -> [ElysiaClawChatMessage] {
        var result: [ElysiaClawChatMessage] = []
    ...
    private static func dedupeKey(for message: ElysiaClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ElysiaClawChatMessageContent] = [
            ElysiaClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> ElysiaClawChatAttachmentPayload in
            ElysiaClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ElysiaClawChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = ElysiaClawChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: ElysiaClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ElysiaClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ElysiaClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ElysiaClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ElysiaClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
