...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["elysiaclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["elysiaclaw", "pollId"]) ??
    readNestedString(value, ["elysiaclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "elysiaclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "elysiaclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          elysiaclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "elysiaclaw poll vote",
          displayText: "Vote recorded",
          value: { elysiaclawPollId: pollId, pollId },
        },
      },
    ],
  };
