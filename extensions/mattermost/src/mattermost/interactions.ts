...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__elysiaclaw_channel_id";
...
type InteractionCallbackConfig = Pick<ElysiaClawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "elysiaclaw-mattermost-interactions").update(botToken).digest("hex");
}
