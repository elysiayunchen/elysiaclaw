...
vi.mock("elysiaclaw/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("elysiaclaw/plugin-sdk/diagnostics-otel")>(
    "elysiaclaw/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { ElysiaClawPluginServiceContext } from "elysiaclaw/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "elysiaclaw/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/elysiaclaw-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): ElysiaClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("elysiaclaw.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("elysiaclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("elysiaclaw.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("elysiaclaw.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("elysiaclaw.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("elysiaclaw.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("elysiaclaw.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("elysiaclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("elysiaclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("elysiaclaw.webhook.processed");
    expect(spanNames).toContain("elysiaclaw.message.processed");
    expect(spanNames).toContain("elysiaclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["elysiaclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("elysiaclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "elysiaclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["elysiaclaw.reason"]).toBe("string");
    expect(String(attrs?.["elysiaclaw.reason"])).not.toContain(
