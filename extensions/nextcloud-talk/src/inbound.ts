...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as ElysiaClawConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as ElysiaClawConfig,
    surface: CHANNEL_ID,
  });
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ElysiaClawConfig);
