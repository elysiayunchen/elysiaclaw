...
`elysiaclaw.invoke --each`) without adding new ElysiaClaw code per workflow.
...
This extension depends on ElysiaClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ElysiaClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.elysiaclaw/extensions` as a standalone plugin directory.
