import type { RuntimeEnv, ReplyPayload, ElysiaClawConfig } from "elysiaclaw/plugin-sdk/tlon";
import { createLoggerBackedRuntime, createReplyPrefixOptions } from "elysiaclaw/plugin-sdk/tlon";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as ElysiaClawConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in ElysiaClaw config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your ElysiaClaw config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.elysiaclaw.ai/concepts/session#secure-dm-mode`;
