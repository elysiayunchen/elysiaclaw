...
import type { ElysiaClawPluginApi } from "elysiaclaw/plugin-sdk/device-pair";
import {
...
function resolveGatewayPort(cfg: ElysiaClawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.ELYSIACLAW_GATEWAY_PORT?.trim()) ??
...
function resolveScheme(
  cfg: ElysiaClawPluginApi["config"],
...
function resolveAuthLabel(cfg: ElysiaClawPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: ElysiaClawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the ElysiaClaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the ElysiaClaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
