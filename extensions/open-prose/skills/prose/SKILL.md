...
metadata: { "elysiaclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == ElysiaClaw `sessions_spawn`
- **File I/O** == ElysiaClaw `read`/`write`
- **Remote fetch** == ElysiaClaw `web_fetch` (or `exec` with curl when POST is required)
