import type { AnyAgentTool, ElysiaClawPluginApi } from "elysiaclaw/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: ElysiaClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<ElysiaClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as ElysiaClawPluginApi,
    resolveTool,
  };
}
