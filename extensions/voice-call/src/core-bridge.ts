...
function resolveElysiaClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ELYSIACLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ELYSIACLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveElysiaClawRoot(), "dist", "extensionAPI.js");
