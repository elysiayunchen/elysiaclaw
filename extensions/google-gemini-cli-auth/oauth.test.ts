...
vi.mock("elysiaclaw/plugin-sdk/google-gemini-cli-auth", () => ({
  isWSL2Sync: () => false,
...
describe("loginGeminiCliOAuth", () => {
  ...
  const ENV_KEYS = [
    "ELYSIACLAW_GEMINI_OAUTH_CLIENT_ID",
    "ELYSIACLAW_GEMINI_OAUTH_CLIENT_SECRET",
  ...
  beforeEach(() => {
    ...
    process.env.ELYSIACLAW_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.ELYSIACLAW_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret"; // pragma: allowlist secret
  ...
  it("falls back across loadCodeAssist endpoints with aligned headers and metadata", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === USERINFO_URL) {
        return responseJson({ email: "lobster@elysiaclaw.ai" });
      }
      if (url === LOAD_PROD) {
