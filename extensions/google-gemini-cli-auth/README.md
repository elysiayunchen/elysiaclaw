# Google Gemini CLI Auth (ElysiaClaw plugin)
...
```bash
elysiaclaw plugins enable google-gemini-cli-auth
```
...
```bash
elysiaclaw models auth login --provider google-gemini-cli --set-default
```
...
- `ELYSIACLAW_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `ELYSIACLAW_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
