...
Read-only diff viewer plugin for **ElysiaClaw** agents.
...
Set plugin-wide defaults in `~/.elysiaclaw/elysiaclaw.json`:
...
Before:
ElysiaClaw supports plugins.

After:
ElysiaClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/elysiaclaw-diffs`).
