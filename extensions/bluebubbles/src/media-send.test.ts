...
import type { ElysiaClawConfig, PluginRuntime } from "elysiaclaw/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): ElysiaClawConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "elysiaclaw-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
