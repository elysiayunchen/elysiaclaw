...
If you believe you've found a security issue in ElysiaClaw, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [elysiaclaw/elysiaclaw](https://github.com/elysiaclaw/elysiaclaw)
- **macOS desktop app** — [elysiaclaw/elysiaclaw](https://github.com/elysiaclaw/elysiaclaw) (apps/macos)
- **iOS app** — [elysiaclaw/elysiaclaw](https://github.com/elysiaclaw/elysiaclaw) (apps/ios)
- **Android app** — [elysiaclaw/elysiaclaw](https://github.com/elysiaclaw/elysiaclaw) (apps/android)
- **ClawHub** — [elysiaclaw/clawhub](https://github.com/elysiaclaw/clawhub)
- **Trust and threat model** — [elysiaclaw/trust](https://github.com/elysiaclaw/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@elysiaclaw.ai](mailto:security@elysiaclaw.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.elysiaclaw.ai).
...
- Tested version details (ElysiaClaw version and/or commit SHA).
...
- Demonstrated impact tied to ElysiaClaw's documented trust boundaries.
- For exposed-secret reports: proof the credential is ElysiaClaw-owned (or grants access to ElysiaClaw-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at ElysiaClaw. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

ElysiaClaw is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
ElysiaClaw does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- ElysiaClaw can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need ElysiaClaw, use one VPS (or host/OS user boundary) per user.
...
- This is expected in ElysiaClaw's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of ElysiaClaw's trusted computing base for a gateway.
...
- Using ElysiaClaw in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.elysiaclaw`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not ElysiaClaw-owned and not granting access to ElysiaClaw-operated infrastructure/services) without demonstrated ElysiaClaw impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

ElysiaClaw security guidance assumes:

- The host where ElysiaClaw runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.elysiaclaw` state/config (including `elysiaclaw.json`) is effectively a trusted operator.
...
ElysiaClaw's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
ElysiaClaw separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when ElysiaClaw can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the ElysiaClaw process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

ElysiaClaw uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/elysiaclaw` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/elysiaclaw` (or `elysiaclaw-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the ElysiaClaw-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use ElysiaClaw temp helpers (`resolvePreferredElysiaClawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-elysiaclaw-dir.ts`
...
For threat model + hardening guidance (including `elysiaclaw security audit --deep` and `--fix`), see:

- `https://docs.elysiaclaw.ai/gateway/security`
...
ElysiaClaw's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `elysiaclaw gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - ElysiaClaw keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `elysiaclaw security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__elysiaclaw__/canvas/`, `/__elysiaclaw__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__elysiaclaw__/canvas/`, `/__elysiaclaw__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

ElysiaClaw requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running ElysiaClaw in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v elysiaclaw-data:/app/data \
  elysiaclaw/elysiaclaw:latest
```
